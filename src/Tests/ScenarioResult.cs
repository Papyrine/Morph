...
/// <summary>
/// Per-page comparison against the Word reference render. <paramref name="ErrorMetric"/> is
/// ImageMagick's Absolute error (fraction of pixels that differ at all; 0 = identical) and
/// <paramref name="Ssim"/> is structural similarity (1 = identical; null when the page sizes
/// differ — see <see cref="PageSsim"/>). They fail differently: AE is blind to structure — a
/// page with headings drawn through body text can score the same as a clean one when most
/// pixels are white — while SSIM is blind to sparse pixel-exact differences. Read them
/// together.
/// </summary>
