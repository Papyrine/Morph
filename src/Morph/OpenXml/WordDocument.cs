...
public sealed class WordDocument
{
    ...
    /// <param name="defaultFont">Optional default font family name override. When null,
    /// <see cref="DefaultFontSettings.DefaultFont"/> is used.</param>
    public WordDocument(string docxPath, string? defaultFont = null)
    {
        using var stream = File.OpenRead(docxPath);
        Document = new DocumentParser(defaultFont ?? DefaultFontSettings.DefaultFont).Parse(stream);
    }

    /// <summary>Loads a DOCX from a stream.</summary>
    public WordDocument(Stream docxStream, string? defaultFont = null) =>
        Document = new DocumentParser(defaultFont ?? DefaultFontSettings.DefaultFont).Parse(docxStream);
