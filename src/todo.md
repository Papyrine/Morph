...
- Flip-gate deficit: the anchor-wiring design is a measured DEAD END in its heuristic form (2026-08-05, third attempt — all code reverted). The full mechanism (AnchorParagraph on the float models, ParseParagraph-tail wiring, Fragmenter deferral + placed-paragraph-tops dictionary for backward resolution) DID land both hand targets — agendas-minutes/05 32.3 → 5.0 (exact production tie) and menus/05 23.2 → 15.3 (beats production) — but the corpus measurement was decisive against the anchor-identification heuristic: −0.0269 aggregate, 64 regressed / 6 improved (wedding/07 −0.19, labels/07 −0.15, cards/03 −0.11, menus/04, newsletters/08, resumes/04). The heuristic (nearest ParagraphElement before the float in its ParseParagraph output, else next, else last) mis-attributes anchors in the dominant emit-floats-first pattern, and the resolved base also shifts by the spacing-before collapse relative to the old cursor approximation. A future attempt needs the TRUE anchor identified structurally at parse time (the drawing's owner paragraph is exactly known when the run parses — record it there, before any flush-ordering ambiguity, e.g. by wiring floats to the FIRST fragment of their owner paragraph and verifying the spacing-reference against a Word probe with controlled spacing-before). Baseline numbers for that probe: cursor 32.3 / prev-top 5.0 / next-top 140.8 / wired+dict 5.0 on agendas; cursor 23.2 / wired 15.3 on menus.
