...
- **#25 Word splits a row that merely does not fit the space left — ATTEMPTED AND REVERTED 2026-08-06, blocked on the fragment's fit rule.** Word-probed (`_probe_cantsplit_fit_off`): 168pt free against a 240pt one-row table, and **Word fills the page to 709pt — 13 of the row's lines — carrying 7 overleaf**, even though the row would have fitted a fresh page. The engine moves it whole, i.e. behaves as though every row carried `w:cantSplit`.
  ...
  **The last-line rule was probed next (`_probe_lastline_*`, 2026-08-06) and it is NOT simply one path being wrong — both readings are refuted.** Sixty paragraphs against a 648pt band, in the flow and in a splitting one-row table, at exact and at auto line spacing:
  ...
  Both constraints together point at a rule between the two: the line's INK box (ascent + descent) must fit, and only the external leading may hang past the margin. That is consistent with every measurement here — under exact spacing there is no leading, so ink box = full box and Word breaks at 49; under auto spacing the extra leading is not enough to admit a 43rd line either way. Testing it needs a descent (or ink-height) on `MeasuredLine`/`LaidOutLine`, which today carry only Height and Ascent, so the measurer has to expose it first. Note the flow and the cell genuinely differ under auto spacing (Word 42 vs 41, which the engine reproduces) — that is a separate difference in a cell's usable height, not the last-line rule, and is uncharacterised.
