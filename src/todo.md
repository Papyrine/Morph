...
12. **TOC rendering broken** — ✅ FIXED (−0.91 summed, all four TOC scenarios, zero collateral). Two independent causes: (a) **page numbers absent** — TOC1's right-leader tab stop (10790 twips = 539.5pt) exceeds its host cell's ~310pt wrap width, and `TabStopResolver` dropped the following text entirely. Word actually CLAMPS a Right/Center/Decimal stop past the wrap width to the wrap width and right-aligns the number at the cell edge (business-plans/12/13 verified against Word's renders). The resolver's old "suppress the following text" rule was calibrated against `table_of_contents/03` — whose entries turn out to have NO text after the tab at all (the fixture's PAGEREFs were stripped), so its dots-to-the-edge-no-number render was vacuous evidence that never distinguished suppress from clamp. Clamp implemented, spec tests rewritten to the clamp semantics. (b) **hyperlink styling** — a `\h` TOC stamps `rStyle=Hyperlink` on every entry run, but Word's print layout suppresses the character style inside TOC entries (links functional, visuals from the TOC N paragraph style). `ParseRunProperties` now drops the Hyperlink character style when the paragraph style is TOC*: no underline, no hyperlink colour — and business-plans/15's inflated line pitch (the underlined runs' leading) collapsed to Word's, its TOC fitting one page again (−0.88; page count 19→17 vs Word's 19 — the two pages the bloated TOC used to add are gone, remaining divergence is the systemic #2 page-count class). Residuals: TOC page numbers are the CACHED field values (correct only as of the doc's last update in Word — live PAGEREF evaluation would need the bookmark→page map); mine's numbers sit ~4pt left of Word's (clamp lands at cell content edge, Word spills slightly into the cell padding); the TOC heading's underline rule and the remaining line-pitch hair are #2-class.
13. **Footnotes/endnotes** — ⚠ MOSTLY FIXED. The reference marks now render in every backend: `w:footnoteReference`/`w:endnoteReference` runs carry the CITATION-ORDER number (1, 2, 3… — footnotes.xml ids start at 2, so the id is never the display number; repeats reuse their number) as superscripted text, drawn by the existing sub/superscript path — the exporters branch on `FootnoteReferenceId` before `Text`, so their own marker/backlink emission is untouched. The PDF backend previously DROPPED the notes outright; it now renders the same document-end "Footnotes"/"Endnotes" appendix as the raster backends (`RenderNotesAppendix` ported to `PdfPageRenderer`), and all three appendices renumber sequentially (was the raw footnotes.xml id — "2." for the first note). document_capture/01's pdf_result +0.002 is the appendix sitting at document end where Word pins the footnote to the page bottom — the known, deliberate deviation (page-bottom pinning needs page-level reservation in layout, unwired); the previously-lost note text now being present outweighs the position noise. **Still open:** page-bottom pinning + the separator rule above the notes; Word's footnote-text style (smaller size) is approximated at 10pt.
14. **Comment markup not rendered** — no balloon, no highlight, no markup-area page shrink (`comments/01`).
15. **Legacy form-field glyphs missing** — ⚠ MOSTLY FIXED. The fields never parsed at all: `w:ffData` nests INSIDE `<w:fldChar w:fldCharType="begin">`, and `ParseFormField` looked for it as a run-level sibling — every legacy field fell through to plain text. With the lookup fixed, the per-type treatment follows Word's PRINT rendering: (a) FORMCHECKBOX becomes an INLINE run ("☐ Option 1" flows on one line) — ☐ U+2610 / ☒ U+2612 drawn into the embedded `Morph Bullets` face (box ≈1.15em dipping below the baseline, sized against Word's render of a 10pt field; Word draws the box larger than the nominal ffData size), the run carrying `FontFamily = "Morph Bullets"` and the ffData half-point size; `form_checkboxes` shows both states at Word's positions (box 22px vs Word's 20px at 150dpi). (b) FORMTEXT/FORMDROPDOWN deliberately DON'T emit their block widgets — Word prints just the cached result text inline ("Name: Enter your name"), which is exactly what falls out of the ordinary field plumbing; the first attempt emitted the widgets and DOUBLE-rendered (chrome box + the cached text, form_text_fields +0.010) because the cached result runs between separate/end still flow. The old accidentally-correct inline rendering is now the deliberate one. **Still open:** `content_control_inline`'s SDT content controls are a different mechanism (w:sdt, not ffData) and still render as block widgets with chrome; the base-renderer widget code (`RenderCheckBoxFormField` etc.) is now unreachable from DOCX parse.
...
21. **Picture rotation** — ✅ MOSTLY FIXED. The PDF backend now rotates inline images (`PdfTextEngine.DrawImage`) and anchored/floating images (`PdfPageRenderer.RenderFloatingImage`) around their centre, matching the raster backends: `image_rotation/01` and `letters/13`'s vertical banner now rotate, and the wedding scenarios' rotated florals now match the raster. (Rotation still reserves the un-rotated footprint, so a rotated image can overlap neighbouring text — a documented, all-backend limitation.) **PDF text boxes now rotate too** — `RotateAtTransform` about the box centre around the chrome + text content, matching the raster backends; `labels/06`'s "ADMIT ONE" captions render vertical (they additionally needed the systemic-#5 sixth-pass z-order interleave to surface from under the ticket art). Issue #21 fully resolved.
...
31. **CSS styling in AltChunk/HTML-source tables dropped** — ⚠ MOSTLY FIXED (−0.10 summed; html_table_styled transformed far beyond its SSIM delta — the blue header band, white bold header text, zebra row shading, 100%-width fill, per-column alignment and fixed column widths all render now). `HtmlParser.ParseTable` gains: row-level `<tr style>` background-color/color cascading onto the row's cells (HTML's header-fill and zebra idiom), cell `text-align` → paragraph alignment, cell `color` → run colour, cell `width` (px/pt/% styles + the legacy width attribute, px×0.75→pt) → `TableCellProperties.WidthPoints`/`WidthFraction`, and table `width` (px→`PreferredWidthPoints`; ≥99% → `FillContainer`, which drives the autofit scale-up). html_complex −0.09, html_css_alignment's vertical-alignment demo table now spreads its three columns across the width like Word (+0.003 is cell-height geometry noise, visually much closer). **Still open:** cell-level inline formatting is flattened (`cell.TextContent` builds ONE run, so `<b>`/`<span style>` inside a cell lose their formatting — the run colour comes only from the row/cell style); cell padding composition (cellpadding attr + per-cell `padding` renders slightly tighter than Word's); vertical-align on cells is unmodelled (html_css_alignment's actual demo intent).
...
- ✅ FIXED | all | p2 | TOC page numbers now right-align at the cell edge on every entry (3,4,5,6,8,10,11,12,15,16,18 matching Word) — TOC1's full-page right-leader tab stop clamps to the host cell's wrap width instead of dropping the number (systemic #12)
- ✅ FIXED | all | p2 | TOC entries no longer underlined and the entry pitch matches Word — the Hyperlink character style is suppressed inside TOC-styled paragraphs as Word's print layout does (systemic #12)
...
- ✅ FIXED | all | p2,p3 | TOC entries regain their right-aligned page numbers and drop the hyperlink underline/spacing (tab-stop clamp + TOC Hyperlink suppression, systemic #12)
...
- ✅ FIXED | all | p2-p18 | the TOC fits Word's single p2 again at Word's line pitch (the hyperlink styling inflated every entry's leading; suppressed per systemic #12, −0.88 across the scenario). Page count now 17 vs Word's 19 — the two bloat pages are gone and the residual divergence is the systemic #2 page-count class.
...
- ✅ FIXED | pdf | p1 | Rotated "ADMIT ONE" edge captions now render vertically at the ticket edges — the PDF backend rotates floating text boxes about their centre (`RotateAtTransform`), and the cross-type group z-order interleave lifted them from under the ticket-body pictures where they had been covered (metric +0.019 is the new-ink penalty for the captions actually rendering; 3-up vetted against Word)
...
- ✅ FIXED | all | p1 | The missing collage shapes render — 27 pieces defer their fill via `a:grpFill` to their cluster group's colour, now resolved through the ancestor chain (systemic #5 seventh pass; the sketchy black frame outlines were the tx1-filled cluster). Art still bleeds to the page edges instead of keeping Word's white margin — the group-extent clip remains open. Superseded census below. CENSUS (2026-07-19, render-stream instrumentation): the XML anchor holds 45 children (8 nested grpSp + 37 wsp: 2 rect, 16 custGeom+solidFill, 19 custGeom noFill outline shapes) but only EIGHT FloatingShapeElements reach the render stream — 10 FILLED shapes drop (the voids) plus all 19 outlines (the sketchy frame borders). Prime suspect for the ShapeParser side: `IsDecorativeShape` ("complex paths with curves") nulls curve-heavy custGeoms in `ParseGroupedShape`; the walk (`ParseSolidFillShape`) has no such filter and should have emitted the survivors, so a second drop exists in the walk path (unresolved — next step: parse-side instrumentation per wsp through both parsers). The page-edge bleed is the missing GROUP-EXTENT CLIP: Word cuts children at the group's frame (the same class as cards/13's stray art); a render-time clip to the anchor extent for group-child batches would restore the white margin.
...
- ✅ FIXED | all | p1 | the stray pink-bear fragment is gone (the second reclassification was also wrong: the bears are 150 custGeom SHAPES in a HEADER group with non-identity nested sub-groups — the fragment was ShapeParser's mis-scaled duplicate, eliminated by the walk-authority rule, systemic #5 tenth pass)
