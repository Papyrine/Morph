...
- **#45 The column-width RULE is now known; landing it is blocked on the fit-to-page scale** (re-probed at A4 on 2026-08-14, superseding the same day's Letter-contaminated reading).

  **Excel's rule, measured.** A declared `col/@width` becomes `width * maxDigitAdvancePx` — the EXACT advance, no rounding, and NO additive padding. Probed on six faces with `printOptions/@gridLines` so the boundaries are drawn rules (`_probe_grid_*`, columns of 6/30/60 characters):

  | font | Excel MDW | max digit advance | ratio | fitted padding |
  | --- | --- | --- | --- | --- |
  | Calibri 11 | 7.519 | 7.434 | 1.011 | −0.11 |
  | Arial 10 | 7.370 | 7.415 | 0.994 | −0.22 |
  | Corbel 11 | 7.685 | 7.691 | 0.999 | −0.11 |
  | Georgia 11 | 8.963 | 9.002 | 0.996 | +0.22 |
  | Segoe UI 11 | 8.000 | 7.906 | 1.012 | 0.00 |
  | Consolas 11 | 8.167 | 8.064 | 1.013 | 0.00 |

  It really is the MAXIMUM digit and not `'0'`: of the six, only Corbel has a digit wider than its `'0'` (7.691 against 7.534), and taking the maximum moves its prediction from 2.0% wide to 0.1%. The 5px padding is already inside the stored width — ECMA-376 §18.3.1.13 writes it as `(chars*MDW+5)/MDW` — so `ToPoints`'s `+5` is a double count.

  **Why it is not implemented.** Written as above it makes the corpus WORSE: SSIM −0.4525 and AE +0.2132 over the 110 pages it moves, wins to +0.13 (`weekly-lesson-planner`) against losses to −0.11 (`social-media-editorial-theme-calendar`). The crops agree with the metric here, unlike the placement fixes above — `home-contents-inventory-list` was already near-exact (grid 1043x339px against Excel's 1041x339) and the change takes it to 1049x343. The cause is that **37 of the 40 workbooks set `fitToPage`**, and for a fitted sheet the grid is scaled to the page, so the MDW barely touches the rendered WIDTH and instead sets the SCALE — which then rescales the row heights. The current integer rounding is calibrated for that: the code comment records `check-register` being tuned so its fit scale reached Excel's 70%. So width and scale have to be re-derived together against Excel, not one at a time. `ResolveScale` itself reads correctly (shrink-to-fit per axis, `min` of the two), so the suspect is what it is fed, or `NaturalHeightPoints`/`autoRowHeightFactor` on the other axis.

  **Method note, and the reason the first attempt inverted the answer.** `RenderHelper` sets `PaperSize = xlPaperA4` over COM and reads it back, and Excel REPORTS A4 while a Letter-paper printer driver silently exports Letter pages — the guard passes, and the A4 layout is squeezed onto Letter. Every measurement taken that way is scaled by roughly 0.92, which is what produced the phantom "Excel's MDW is ~0.92 of the font advance, per-font and unexplainable" reading and sent two hypotheses (GDI cell-height, hinted advances) down blind alleys. Column widths are paper-independent in principle, so this was invisible until the same fixture was rendered both ways: 42/207/415px on Letter against 45/226/451 on A4. **Confirm the printer really is A4 by rendering one fixture and checking the page comes out 1123x794 (A4 landscape at 96 DPI), not by trusting `Get-PrintConfiguration`** — that cmdlet reported Letter on this machine while Excel was exporting A4.

  **Until it closes, SSIM misreads spreadsheet text-placement fixes.** Both cell-overflow changes of 2026-08-14 scored NEGATIVE while being visibly correct against `expected_0001.png` — single-line at −0.00005 aggregate and the clip at −0.0013 — because correct placement stopped masking the width error. Read the crops, per `docs/fidelity-audit.md`; do not revert a spreadsheet placement change on the number alone.
