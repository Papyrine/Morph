...
- **#25 `w:cantSplit` is unimplemented** (found 2026-08-06). A row marked `w:cantSplit` must move whole to the next page rather than split across one; the engine has no handling at all, and `PlaceTableRowByRow` already moves rows whole, so the gap only shows where a row WOULD otherwise split. No corpus document uses it, which is why it has never surfaced.
...
- **#2 Page-count divergence on long documents — the three-way divergence is OVER.** With the layout-engine PDF flip (2026-08-06) all three backends paginate through the one `Fragmenter`, so no scenario disagrees *between* backends anymore. The recount is **Skia 323, ImageSharp 323, PDF 323 of 325** (2026-08-06, after `image_wrap_square` was closed by the continuous-section-break mark rule — see `src/page_counts.md`), with two scenarios differing from Word identically on every backend: `business-plans/15` (cumulative drift, not a rule) and `newsletters/06` (+2, a near-full-page `atLeast`-table knife-edge — its last row bumps off a table that clears the page by 13pt). `resumes/13` — the old archetype (raster short / PDF over / Word between) — now renders Word's 5 pages on all three. Root-cause taxonomy, the experiment ledger and the `newsletters/06` breakdown are in `src/page_counts.md`; per-scenario "drifts up"/"compresses" findings below predate the regenerated baselines and are unverified.
