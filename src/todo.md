...
- **#46 Paragraph border box sits ~1pt inside Word's** (the stroke-direction half LANDED 2026-08-29; see `docs/word-features.md`, Paragraph Borders). With the stack now stroking outward, Morph's left rule has the same invariant inner edge Word's does but 2px in from it: Word's border box is about 1pt OUTSIDE the nominal (text margin − `w:space`), on all four sides. Swept over `w:space` 0/2/4/8/12/20/28pt at 6pt width (`_probe_pbdrspace`, 150 DPI) the implied constant is 0.96 / 0.88 / 0.80 / 0.64 / 1.44 / 1.12 / 0.80pt — flat, no trend with space, mean 0.95pt, the scatter being pixel quantisation. Landing it moves the painted BOX outward 1pt without changing the flow reserve (Morph's text already sits where Word's does — both start at x=152 in the probe), so it is paint-only like the stroke fix was, but it re-judges every `w:pBdr` scenario again and should be adjudicated on its own.
...
- MINOR | all | p1 | cumulative downward drift to ~49px by the page bottom against matching band COUNTS — consistent with Morph's 278/240 x 14.65pt Calibri line (16.97pt) against Word's grid-snapped 16.8pt (= 28 whole 120-dpi pixels), ~0.17pt per line; a line-pitch quantisation question, not spacing rules
