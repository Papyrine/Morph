...
- Flip-gate deficit root-caused; anchor-wiring prototype measured (2026-08-05, second session — all code reverted, ledger only): adding `AnchorParagraph` to the float models, wiring it at ParseParagraph's tail (nearest paragraph before the float in the call's output, else next, else last) and deferring paragraph-anchored floats in the Fragmenter until their anchor places IMPROVED menus/05 beyond production (23.2 → 15.3px mean band error) — the design works where the wiring reaches. agendas-minutes/05's shapes never received an anchor (a Console probe in the resolver fired zero times): they are parsed by a path that does not flow through ParseParagraph's tail — find that path (group/DrawingML? the z-sort walkSurvivors at DocumentParser ~5512?) and wire it too. Unexplained: with null anchors the fallback should equal the old cursor behaviour, yet agendas measured 80.2px vs the 32.3 baseline — diff the reverted refactor against the old inline emission before re-attempting (suspect: emission timing relative to band resolution/exclusion registration). Baseline numbers: cursor 32.3, prev-paragraph 5.0 (but menus −0.037), next-paragraph 140.8.
