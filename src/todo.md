...
- **#25 Table rows never split across a page** (found 2026-08-06). `PlaceTableRowByRow` moves a row that will not fit whole to the next region and `LayoutCellContent` takes no page breaks, so a row taller than the content height overflows off the page instead of continuing on the next. This is `business-plans/15`'s whole page-count gap (`src/page_counts.md`), and it makes `w:cantSplit` moot — that attribute selects the behaviour the engine already has unconditionally. Closing it means breaking cell content at a line boundary and re-emitting the row's borders, shading, vertical alignment, spans and `w:tblHeader` repeats per fragment.
...
- **#2 Page-count divergence on long documents — the three-way divergence is OVER.** With the layout-engine PDF flip (2026-08-06) all three backends paginate through the one `Fragmenter`, so no scenario disagrees *between* backends anymore. The recount is **Skia 324, ImageSharp 324, PDF 324 of 325** (2026-08-06, after `image_wrap_square` was closed by the continuous-section-break mark rule and `newsletters/06` by making an authored `w:tblGrid` authoritative under autofit — both in `src/page_counts.md`). One scenario still differs from Word, identically on every backend: `business-plans/15`, and it is now root-caused — **the engine never splits a table row across a page**, while that document wraps whole prose sections in one-row tables (one is 23 paragraphs). The row overflows the page instead of continuing on the next, saving a page. See `src/page_counts.md`; row splitting is also the prerequisite for #25. `resumes/13` — the old archetype (raster short / PDF over / Word between) — now renders Word's 5 pages on all three. Root-cause taxonomy, the experiment ledger and the `newsletters/06` breakdown are in `src/page_counts.md`; per-scenario "drifts up"/"compresses" findings below predate the regenerated baselines and are unverified.
