...
- **#25 Word splits a row that merely does not fit the space left; the engine only splits one taller than a whole region** (Word-probed 2026-08-06, `_probe_cantsplit_fit_off`). 40 filler lines then a one-row table of 20 lines, with 168pt left on the page and the row needing 240pt: **Word fills page 1 to 709pt — 13 of the row's lines — and carries 7 overleaf**, even though the row would have fitted a fresh page. The engine moves it whole instead, i.e. it behaves as though `w:cantSplit` were always set. Deliberately not changed alongside the `cantSplit` work: widening the split trigger is a far broader behavioural change, and the corpus is at 325/325 page counts with the present rule, so it needs its own gate and its own adjudication rather than riding along. The probe fixtures and the exact figures are in this entry; `w:cantSplit` itself is now read and honoured, and the widow/orphan half of this entry is closed (both paths settle the two rules in Word's order).
