...
7. **Text inside dark shapes renders black instead of its white/light run color** — menus/03/07/08/09, inline_group_crop, inline_group_rotation, brochures/03 (title + body on navy). Barely-legible or invisible output.
...
23. **Small caps not applied** (`small_caps`, `feature_capture/01`), right-alignment of `w:bidi` paragraphs lost (`rtl_paragraph`), soft hyphens drawn as visible hyphens (`hyphenation_soft`), non-breaking hyphen U+2011 renders as tofu with overlap (`hyphenation_nonbreaking`, brochures/01 phone number).
...
- MAJOR | all | p1 | title "Technology For all" rendered black instead of white on the navy background
- MAJOR | all | p2 | "Relecloud" title and the "Founded in 2001..." body paragraph rendered black instead of white (near-invisible on navy; subtitle color is correct)
...
- MAJOR | all | p1 | balloons photo missing from the yellow right panel (area left plain yellow)
...
- MAJOR | all | p2,p3,p4,p5,p6 | Footer page number (small grey numeral bottom-left, "2".."6") is missing on every body page in all three backends — bottom strip verified completely blank vs Word.
- MAJOR | all | p1,p2,p3,p4,p5,p6 | Section-marker arrow glyph is wrong: Word draws a small thin chartreuse "→" (~25px), all backends draw a much larger bold arrow with an oversized chevron head sweeping ~3x the height, at every section heading and in the footer contact rows.
...
- MAJOR | all | p1 | decorative corner graphics (top-right and bottom-left orange/mauve staircase shapes) missing in all three backends
...
- MEDIUM | all | p1 | "Celebrate!" captions aligned to the green panel's left edge instead of centered beneath it (~73px left of Word)
...
- MEDIUM | all | p2 | inside placeholder text box ~40px narrower than Word — text wraps to 6 lines vs 5 and renders left-aligned instead of centered
...
- MAJOR | all | p1 | scroll-banner picture missing from both tickets; its three stars render as three orange squares, and on the lower ticket the squares sit at the ticket's top edge instead of mid-ticket
...
- MEDIUM | all | p1,p3,p5,p7 | season caption ("Autumn"/"Spring"/"Summer"/"Winter") rendered left-aligned at the card's left edge instead of centered under the picture (~1.8in left of Word's position)
- MEDIUM | all | p2,p4,p6,p8 | centered placeholder paragraph rendered left-aligned — its short last line "your own or delete for a blank card." sits at the left margin instead of centered (~0.6in left)
...
- MEDIUM | all | p1 | "Celebrate!" captions rendered flush-left at the card's left edge instead of centered under the picture (~155px left of Word); card 2's caption also sits ~90px higher
...
- MAJOR | all | p1 | green card-background shapes drawn grossly oversized — and rotated ~45° into diamonds in the right column — overflowing their cells and overlapping neighbouring cards instead of Word's neat notched rectangles; white card text partially falls off the green (leading "D" of DAVIDE MILANO unreadable on right-column cards)
- MAJOR | all | p1 | blue/grey corner triangles drawn several times larger than Word and misplaced across the sheet instead of small clipped accents at each card's left edge
...
- MEDIUM | all | p1 | "Celebrate!" captions rendered flush-left at the card's left edge instead of centered under the picture (~155px left of Word); card 2's caption also sits ~85px higher
...
- MEDIUM | all | p1,p2 | Text Word centers in the card is rendered flush-left: "THANK YOU" starts ~104px left of Word and overflows past the pink card's left edge onto the white margin (p1), and the pink message on p2 is left-aligned instead of line-centered; both blocks also sit ~20px higher
...
- MINOR | html | - | THANK YOU headings and messages left-aligned instead of centered in their cells
...
- MEDIUM | all | p1 | "Celebrate!" captions left-aligned at the square's left edge instead of centered beneath it (~158px left of Word), both cards
...
- MEDIUM | html | - | centered paragraphs render flush-left: "Merry Christmas" headings and all six "We hope you have a wonderful holiday!" messages are left-aligned in their panels instead of centered
...
- MEDIUM | all | p1 | the italic "x" paragraph renders at the left margin instead of horizontally centered as in Word
- MINOR | html | - | the italic "x" line is left-aligned instead of centered (notes and reference marks otherwise correctly exported)
...
- MAJOR | pdf | p1 | a:srcRect centre-50% crop ignored — full source image drawn scaled into the frame (small centered "Sample", gradient running to near-white) instead of the enlarged cropped centre
...
- MAJOR | all | p1 | all menu text rendered black instead of Word's white/cream (Menu title, Appetizer/Main Course/Dessert headings, body copy) — barely legible against the dark board
- MAJOR | html | - | same wrong text color in HTML export — all board text black on the dark chalkboard instead of white
...
- MAJOR | all | p1 | all text rendered black instead of white ("Menu" loses its white+mint outlined style; headings/body are low-contrast on the dark panel)
...
- MAJOR | html | - | all text black instead of white on the dark panel
...
- MEDIUM | html | - | ticket text left-aligned instead of centered within each ticket
...
- MAJOR | all | p1 | purple brush-stroke background image missing on all 30 labels (only thin slivers of its bottom edge render), leaving the white label text invisible — page appears essentially blank
...
- MAJOR | all | p1 | purple curved-arrow clip-art rendered vertically flipped (curves down-right instead of up-right) in all five purple-arrow labels
- MAJOR | html | - | same purple arrow rendered vertically flipped in the HTML export
...
- MAJOR | all | p1 | entire label artwork missing (navy background with lavender/pink/yellow wave shapes on all 6 labels); white text is drawn on white so the page renders completely blank
...
- MAJOR | all | p1 | "Menu", "Appetizer", "Main Course", "Dessert" rendered with black fill instead of white chalk lettering on the dark board
...
- MAJOR | html | - | headings and "Describe your..." captions black instead of white on the dark board
- MAJOR | html | - | inner decorative frame and mint wavy separators missing

### menus/08

- MAJOR | all | p1 | "EVENT TITLE" and section headings (APPETIZER, FIRST COURSE, MAIN COURSE, DESSERT) rendered near-black instead of white — barely legible against the navy background
- MEDIUM | all | p1 | menu item lines ("List or describe...", "Appetizer item") render dim gray instead of white
...
- MAJOR | html | - | EVENT TITLE, section headings and EVENT INTRO/EVENT DATE labels black instead of white on navy
- MEDIUM | html | - | item text dim gray instead of white and the left menu column loses its centered alignment

### menus/09

- MAJOR | all | p1 | "Menu", "Appetizer", "Main Course", "Dessert" rendered with black fill instead of white chalk lettering
...
- MAJOR | html | - | headings and captions ("Home cook name...", "Describe your...") black instead of white on the dark board
...
- MAJOR | all | p1,p2,p3,p4 | All decorative hexagon shapes render as rectangles/squares (blue-filled hexes, thin blue-outline hex, white hexagon outlines over photos, black outline hex, and the small hex beside "INSIDE THIS ISSUE" all drawn as axis-aligned boxes)
...
- MAJOR | pdf | p1,p2 | watercolor header washes distorted into streaky, oversaturated horizontal bands with wrong shape/extent (p1 right panel bleeds down behind "PARENTS OF THE BRIDE"; p2 both MENU and TABLE panels)
...
- MAJOR | all | p2,p3,p4,p5 | footer page number ("2"–"5", bottom center-right) missing in all three backends
