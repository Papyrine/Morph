...
- MINOR | all | p2 | the large decorative quote mark (") before the yellow-panel quote block is missing (photo/panel layout now matches Word)
...
- ✅→MEDIUM 2026-07-19: the frame renders — its header stacked an opaque white JPEG OVER the frame PNG in document order; header/footer floats now get the body's relativeHeight z-sort. **Residual:** the frame PNG's a:duotone resolves to a light peach where Word shows dark brown (duotone colour-mapping defect, #8 class; metric a wash at +0.001/+0.002 because right-geometry-wrong-colour scores like absent)
