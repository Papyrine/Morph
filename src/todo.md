...
- **#43 Morph's Calibri renders 2.37% narrow at 12pt, which blocks the default-font family fix** (found 2026-08-13). Two separate findings, the second gated on the first:
  ...
  - **The blocker**: Morph's Calibri 12 measures 617px against Word's 632px on the same string — 2.37% narrow — while Morph's Aptos 12 (−1.05%), Times 12 (−0.48%) and Calibri 11 (+0.53%) are all within ~1%. The error is size-dependent within one face, which is the odd part and the thing to probe: Morph scales Calibri 11→12 linearly (1.0920, i.e. 12/11) where WORD scales it 1.1246. Close that, then re-run the family switch and expect both halves to improve together.
