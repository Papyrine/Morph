...
- **#25 `w:cantSplit` is unread** (2026-08-06). Table rows split across pages when a row is taller than a whole region (`src/page_counts.md`). `w:cantSplit` selects move-whole for a row that would otherwise split — implementable now that rows split, but the parser does not read it. No corpus document sets it, so closing this needs a Word probe to confirm the attribute's effect rather than a corpus scenario. (The widow/orphan half of this entry is CLOSED: both the flow path and the cell splitter now settle the two rules in Word's order.)
