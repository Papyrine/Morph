...
- **#25 Word splits a row that merely does not fit the space left — ATTEMPTED AND REVERTED 2026-08-06, blocked on the fragment's fit rule.** Word-probed (`_probe_cantsplit_fit_off`): 168pt free against a 240pt one-row table, and **Word fills the page to 709pt — 13 of the row's lines — carrying 7 overleaf**, even though the row would have fitted a fresh page. The engine moves it whole, i.e. behaves as though every row carried `w:cantSplit`.
  ...
  The likely blocker is visible in the same probe: with the trigger widened, the engine put ONE 12pt LINE MORE on the page than Word (719.5pt against 709.4) in both split fixtures. The cell fragment's budget runs to the content bottom exactly and its fit test uses the full line height, while `PlaceParagraph` tests the ascent and lets the descent encroach the margin — so the two paths disagree about the last line of a page. Under the narrow trigger that error fires on the handful of rows taller than a region; widened, it fires at every table break and compounds. **Settle the fragment's last-line rule against Word first, then re-attempt the widening — not the other way round.**
