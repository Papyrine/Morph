...
4. **Default line-spacing multiplier when no `w:spacing/@w:line` is declared** — ATTEMPTED AND REVERTED 2026-07-21. `DocumentParser` invents **1.04** for a style with no base (line ~1422) and **1.08** for the no-style fallback (`defaultLineSpacingMultiplier`); OOXML says an absent `w:line` is SINGLE, i.e. the font's own line box, which is exactly what `line.Height` already carries. `builtInLineSpacingMultiplier` (278/240) is NOT in question — it is evidence-backed against `long_paragraph` and only fires when a document declares no styles.xml or no docDefaults.
   ...
   **Prerequisite before re-attempting:** `agendas-minutes/07` is already named in `defaultLineSpacingMultiplier`'s own comment as a counterexample for cascading docDefaults' `w:line` (which is deliberately not read), and it is the single biggest winner from 1.0 — strong evidence the two are the same underlying problem. Decode the docDefaults `w:line` cascade and the table-cell rule FIRST, then the invented fallbacks can go. Changing the constant alone just moves which documents are wrong.
