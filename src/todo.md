...
- MEDIUM | html | - | every photo is MISSING from the export, not mis-coloured: the file contains zero `<img>` tags because `HtmlExporter.WriteShape` emits no shape image fills, and all five of this template's pictures are `wps:spPr/a:blipFill` (measured 2026-08-29). A general HTML-export gap that happens to surface here
