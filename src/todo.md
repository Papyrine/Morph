...
- **#45 Spreadsheet geometry: five inputs measured, four of them trustworthy, and the combination still loses to the mutually-calibrated constants** (probed at A4, 2026-08-14). Five implementation rounds, each reverted. Every rule below is reproducible from the probe fixtures; the corpus figures are what stopped each one landing.
  ...
  **Rule 5 — REFUTED. There is no universal row-height factor, and the probe fixtures are unrepresentative.** `_probe_ht` at A4 with printed gridlines gives 20pt -> 25px, 40pt -> 50px, 60pt -> 75px, all exact: 1.25px per point rather than 4/3, a factor of 15/16. Applying it to the corpus is catastrophic (SSIM -1.3702, 29 pages better against 70 worse), and the reason is now known: **a REAL workbook renders declared heights at about 1.05, not 0.9375.** `_probe_real_hh` — `household-organizer` with its fit scale neutralised and gridlines forced on, nothing else touched — draws declared rows of 27.75 / 37.5 / 30 / 30pt as 39 / 53 / 42 / 42px, ratios 1.054 / 1.060 / 1.050 / 1.050, where 4/3 predicts 37 / 50 / 40 / 40. The same declared 30pt is 42px in a real workbook and 38px in the synthetic fixture, so the factor is a property of the PACKAGE, not of Excel.

  The variable is now ISOLATED: **Excel's rendered row height depends on the workbook's BODY FONT, even for `customHeight` rows.** Three candidates were tested by grafting one part at a time onto the same fixture sheet:

  - `printerSettings1.bin` (the DEVMODE blob every real workbook embeds and the fixtures lack): **excluded** — grafting `simple-list`'s part, relationship and content-type override changes nothing to the pixel.
  - `sheetFormatPr` in all five shapes (`defaultRowHeight` 15 / 13.9 / `30 customHeight="1"` / with `x14ac:dyDescent` / element absent): **excluded** — all five render identically (`_probe_fmtpr_*`).
  - `styles.xml` + `theme1.xml` (`_probe_fontgraft`, sheet byte-identical to the fmt15 fixture): **this is it.** With the fixture's Arial 10 body, declared 10/15/20/30/40/60/80pt rows draw 13/18/25/38/50/75/99px (~15/16); with `household-organizer`'s Verdana-11 theme grafted in, the SAME sheet draws 14/20/27/40/54/80/107px — ratios 1.050/1.000/1.012/1.000/1.012/1.000/1.003, i.e. essentially exact.

  That resolves the corpus contradiction completely: the corpus workbooks carry theme fonts whose factor is ~1.0, so the implicit 1.0 in the current code is RIGHT for them and the 15/16 taken from an Arial-10 fixture was a per-font artefact. The real `household-organizer` measuring ~1.05 rather than 1.00 is the remaining 5% — plausibly its non-`customHeight` rows being re-auto-sized at open, worth one probe if that gap ever matters. The likely mechanism (unverified) is Excel's print scaling being derived from the Normal font's metrics on screen vs printer device, which would make the factor a GDI hinting artefact of the body font — measure per face if it is ever needed; do not model it as a constant.

  **This also taints every probe in this entry**, all of which are built by cloning the same minimal hand-made package. Rules 1-3 are corroborated by the corpus moving the right way, so they survive; but re-verify any future probe against a REAL workbook with its scale neutralised before trusting it, as `_probe_real_*` does.
