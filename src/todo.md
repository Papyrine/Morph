...
- **#10 Content-driven autofit.** The conditional run-property cascade has now landed on tri-state modelling (`DeclaredRunProperties`; see "Conditional Formatting" in `docs/word-features.md`), so the remaining half of this item is autofit alone — and the cascade landing has made it visible rather than theoretical. `business-plans/15` p14 is the case: with the `w:caps` toggle finally applied the header row reads ALL CAPS as Word renders it, but "ANNUAL %" no longer fits Morph's column and wraps to two lines where Word keeps it on one, growing the row and carrying +0.1277 of AE for the scenario. Columns are still distributed by a heuristic rather than measured content: on the `Detail` summary table Word's widths run 126.7 / 45.1 / 56.6 / 121.4 / 44.2 / 56.6 / 31.7pt against Morph's 115.2 / 51.8 / 66.7 / 108.0 / 43.7 / 62.4 / 34.1 — same total, wrong distribution, long-content columns under-weighted.
