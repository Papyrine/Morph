...
- **#45 Spreadsheet geometry: the measured width/scale/range rules LANDED 2026-08-14; the residuals below are what is still open.** The landed rules live as code comments on `SheetGridBuilder.MaxDigitWidth` / `ToPoints` and `SpreadsheetParser.ResolveRange` / `ResolveScale`, pinned by `ColumnWidthTests`: column width is `width * maxDigitAdvancePx` (exact advance, MAXIMUM digit not the zero, no `+5` — it is already inside the stored width per ECMA-376 §18.3.1.13), the fit scale floors to a whole percent, `dimension` intersects with the cells that actually exist, and `SheetGeometry` shares the measured unit so anchored art tracks the grid. Landing measured SSIM −0.3921 / AE −0.3298 over 165 raster pages with no page-count change on any backend; the SSIM sign is the known unreliability of that metric on this subsystem (below), the AE improvement and the probe evidence are why it landed. Still open:
  - **The residual geometry split.** Roughly 40% of corpus pages improved and 50% got worse by SSIM at the landing; the worst movers (`social-media-editorial-theme-calendar` −0.10, `home-contents-inventory-list` −0.09) deserve the extents-and-crops treatment to name the next wrong quantity. The row-height model is NOT the suspect for corpus books — see next item.
  - **Row heights render at a BODY-FONT-dependent factor** (isolated 2026-08-14, `_probe_fontgraft`): the same declared heights draw at ~15/16 under an Arial 10 body and essentially exactly under a Verdana-11 theme. The corpus carries theme fonts, so the current implicit 1.0 is right for it; an Arial-10-bodied workbook renders its rows ~6% tall. Likely mechanism (unverified): Excel's print scale derived from the Normal font's GDI metrics on screen vs printer. Measure per face if ever needed; do not model a constant. `household-organizer` measuring ~1.05 against the graft's 1.00 is the one unexplained residue — plausibly non-`customHeight` rows re-auto-sized at open.
