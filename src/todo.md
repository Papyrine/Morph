...
- **#25 Row splitting has no widow/orphan control, and `w:cantSplit` is still unread** (2026-08-06). Table rows now split across pages when a row is taller than a whole region (`src/page_counts.md`), which closed `business-plans/15`. Two follow-ups: the cell splitter does not apply the widow/orphan rule `PlaceParagraph` uses for the page flow, so it can leave a single line overleaf where Word moves the whole paragraph (visible on business-plans/15 page 16, page counts unaffected); and `w:cantSplit` — which selects move-whole for a row that would otherwise split — is now implementable but not yet read by the parser. No corpus document sets it.
