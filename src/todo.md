...
- **#45 A spreadsheet's COLUMNS are 2-8% wider than Excel's** (found 2026-08-14; the grid-ORIGIN half of this entry landed the same day and is gone). Ink extents at 96 DPI over the 40 workbooks: `check-register` +7.1%, `simple-basic-pink-blue-timesheet` and `simple-blue-timesheet-invoice` +7.3%, `simple-purple-credit-note` +7.5%, `simple-green-debit-note` +8.5%, `probate-inventory` +11.8% — against a cluster that is exact (`simple-basic-blue-standard-invoice` +0.0%, `inventory-list` −0.2%, `home-contents-inventory-list` +0.2%), so it is a rule that is wrong for some fonts rather than a constant that is wrong for all. A too-wide grid does not merely misdraw the columns: it eats the centring slack, so those sheets stay hard against the left margin where Excel centres them — which is why the origin fix moved only 7 of the 38 centred workbooks. Prime suspect is `SheetGeometry`'s hardcoded `maxDigitWidthPixels = 7`, which contradicts the measured `SheetGridBuilder.MaxDigitWidth` on the same workbook; that class's own comment says the two must agree cell for cell or anchored art lands off the grid, and they do not. `ToPoints`'s truncation is the second. Probe it the amplified way — one fixture per candidate rule across several body fonts, widths read off the reference — rather than fitting a constant to one corpus sheet.
  - **Until it closes, SSIM misreads spreadsheet text-placement fixes.** Both cell-overflow changes landed on 2026-08-14 scored NEGATIVE while being visibly correct against `expected_0001.png` — single-line at −0.00005 aggregate and the clip at −0.0013 — because correct placement stopped masking the width error. `to-do-list-for-projects` p2 is the live example after the centring fix (−0.0094 on both backends, the one page of 26 that got worse): its grid is 15% NARROW, so its slack is too big and centring pushes it past Excel rather than onto it. Read the crops, per `docs/fidelity-audit.md`; do not revert a spreadsheet placement change on the number alone.

- **#46 Vertical print centring is unprobed past the FIRST page** (opened 2026-08-14, with the feature). `printOptions/@verticalCentered` now centres each page's content between the margins at page close (`Fragmenter.FinishPage`), which is Word-confirmed for a sheet that fits: `simple-green-black-timesheet-invoice` lands 210/231px against Excel's 209/232, and `org-charts-visual` reproduces the asymmetry its 0.5in/1in margins demand. What no reference covers is a sheet that OVERFLOWS its page vertically — all ten centred corpus sheets fit, `to-do-list`'s second page being a horizontal split of the same rows. The implemented rule centres each page on its own slack, so full pages do not move and only the short last page does; Excel may instead centre nothing once the print area spans pages. Settling it needs one probe with a deliberately short last page, and the Print Spooler was stopped when this landed, so `RenderHelper` could not run — see "Excel references need an A4 printer" in CLAUDE.md before trying.
