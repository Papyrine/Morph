...
- **#25 Word splits a row that merely does not fit the space left — the SPLIT MECHANICS are fixed (2026-08-07), the TRIGGER is reverted for the second time.** Word-probed (`_probe_cantsplit_fit_off`): 168pt free against a 240pt one-row table, and **Word fills the page to 709pt — 13 of the row's lines — carrying 7 overleaf**, even though the row would have fitted a fresh page. The engine moves it whole, i.e. behaves as though every row carried `w:cantSplit`.

  **Three defects in the EXISTING split path were found and fixed first, by measuring `_probe_cantsplit_tall_off` (a row that already splits) against Word rather than assuming that path was right.** Each was a case of the fragment not modelling its own chrome:

  | defect | Word | before | after |
  |---|---|---|---|
  | the fragment budget ignored its own horizontal edges | 53 lines | 54 | 53 |
  | split edges drew no rule | rules at 708.96 and 72.00 | none | both |
  | a continuing fragment stretched to the region bottom | closes at 708.96 | 719.52 | 708.48 |

  The budget one is the blocker the first attempt died on (recorded then as "one 12pt line more than Word, 719.5 vs 709.4"): `TableHeightCalculator`'s collapse pass charges an unsplit row for its outer top and bottom and insets content below the top edge, but `BuildRowFragment` charged neither and handed the cells the whole of `contentBottom − y`. Against a 648pt region and 12pt lines, 54 lines fit that unpaid budget exactly where Word's 647 takes 53. Two code comments asserted the opposite of Word on the other two — that a split edge draws no rule, and that a continuing fragment fills its page — and both are corrected against Word's own render.

  **Bisected, because the three do not stand alone.** Adjudicated full-resolution against Word over the 27 changed pages: budget alone is corpus-inert (suite green, zero diffs); borders alone are WORSE (5.943 → 6.629, 15 pages worse), because a closing rule on a fragment still stretched to the region bottom lands ~11pt below the content it closes; borders plus the tight box are byte-identical to the tight box alone, so the rules are corpus-INERT once the geometry is right. Final: **5.943 → 5.807, 16 better / 3 same / 8 worse**, page counts still 325/325/325. The rules are kept because the probe requires them (three of the four fixtures now match Word exactly on line counts and rule positions) and they cost nothing. The two all-backend regressions — `business-plans/13` p16 (+2.66) and `business/03` p2 (+0.97) — come from the geometry, not the rules, and sit against `header_row_repeat/01` p2 (−3.0) and three other `business-plans/13` pages.

  **The trigger itself is reverted again — attempt two, and it now fails on ONE document rather than 129 pages.** Routing a table that will not fit the space left through the row-by-row path (plus splitting a row that will not fit) makes `_probe_cantsplit_fit_off` match Word exactly, and costs three documents. Two causes were isolated: `table_layout_tall_row` broke because the routing tested a strict `>` where `EnsureSpaceFor` allows a 2% overspill, so tables that legitimately stayed were routed — reusing `HasSpaceFor` cleared it, as did excluding `w:hRule="exact"` rows from the row-level trigger (their height is a verbatim box, so splitting lets sparse content collapse into the space left and the reservation vanishes). **`resumes/06` (3 → 6 pages) is the remaining blocker.** A page trace shows pages 2/4/6 each holding exactly one `PlacedTableRow`.

  **That row was first written up here as BLANK, kept alive by `HasVisibleContent` counting any `PlacedTableRow` as content. Tracing the row itself refuted that (2026-08-07): it is row 11 of 12, 67.46pt, and its second cell holds a NESTED TABLE whose cells carry `w:shd` fill `000000` — a black bar, genuinely visible content.** `RowHasVisibleContent` is right to return true for it, and `HasVisibleContent` is not the culprit.

  **A follow-up claim that the orphaned fragment "loses its nested table's shading" was then ALSO refuted by measurement (2026-08-07): the orphan page's only ink band is a SOLID dark bar at x 310–569pt — byte-matching the baseline's bar — sitting at 54–71pt because this document's top margin is 1080 twips = 54pt, not the 72pt assumed when the band was dismissed as "header only".** The orphaned row renders perfectly; there is no rendering bug here at all. The whole remaining question is pagination: Word keeps that 67pt row on the content page where the widened routing breaks it to a new one — presumably squeezing or overflowing it — and that question lives inside the parked trigger widening above, not as an independent defect.

  A suspected latent defect in `RowHasVisibleContent` was chased here and **Word refuted it TWICE — the second probe de-confounded** (2026-08-07). The predicate short-circuits on any non-paragraph cell element, so a row whose only content is an EMPTY, unbordered nested table counts as visible; recursing into the nested table to decide looked like the obvious fix. The first probe (`_probe_trailing_*`) appeared to refute it, but its fixtures ended `</w:tbl><w:sectPr>` with no paragraph after the table — a Word body must end in a paragraph mark, Word repairs that silently, and the blank continuation page then reads equally as "row carried" and "row absorbed, repaired final paragraph took the page". `_probe_trail2_*` de-confounds: a text paragraph FOLLOWS the table and the trailing row carries an explicit 100pt `w:trHeight`, so the row's fate is the AFTER paragraph's position on the continuation page. Same shared geometry (108 borderless rows of a 12pt exact line filling two 648pt bands exactly, over the 110% row-by-row threshold — at 55 rows the whole-table path takes it and tests nothing):

  | trailing 100pt row holds | Word AFTER ink | engine before | engine now |
  |---|---|---|---|
  | an empty paragraph | 174.72pt | 75.36 | 75.36 (open) |
  | an empty nested table | 174.72pt | 99.36 | **175.20** |
  | a bordered nested table | 72–85 ink + 174.72pt | 101.28 | **72–86 + 175.20** |

  **Word carries all three — 174.72 = margin + the full declared height — so the short-circuit reproduces Word for both nested cases and the recursion would have turned a matching case into a failing one.** Pinned by `TrailingRowVisibilityTests`, which carries both probes' numbers and the confound.

  **The probe also caught a regression the kept row-level split trigger had introduced, now FIXED: a row routed through `PlaceSplitRow` and carried whole to a region top was placed as a content-sized fragment, silently dropping its `w:trHeight` floor** (the 99.36/101.28 column). `BuildRowFragment` now floors a first-fragment, whole-row, region-top placement at the AUTHORED height (175.20 vs Word's 174.72, one pixel), pinned by `CanonicalFragmenterTests.A_carried_row_keeps_its_declared_height`.

  Getting the floor's scope right took three corpus adjudications, and the record matters because each widening looked plausible: (1) swapping in the calculator's full `rowHeights` model regressed 16/21 changed pages including `header_row_repeat/01` and `table_multipage`, which author NO trHeight — the fragment path's actual layout beats the model's estimate, so only the authored value may floor; (2) the authored floor alone: `business/03` improved on all three backends (−0.8..−1.2, its floored rows land at region tops, the probed geometry) while `business-plans/13` worsened (+0.35..+0.73 on p14/p16/p20, all backends); (3) gating on region-top placement changed NOTHING for bp/13 — its floored rows are all at region tops too, i.e. exactly the probed geometry. The floor is kept on the probe's direct evidence, and both scenarios' baselines regenerated. **This is also where my "the row-level trigger is corpus-neutral" claim died: page counts held, but the trigger had been rerouting these boundary rows into content-sized fragments, and that movement was silently folded into the 5.943 → 5.807 adjudication.** Neutral-on-page-counts is not neutral.

  **The `business-plans/13` residue was then dug and root-caused (2026-08-07): not row heights at all — the continuation pages were MISSING THEIR REPEATED `w:tblHeader` ROWS.** Reading the actual pages (not band aggregates) showed the engine's p16 starting bare at the "Rent" data row where Word's opens with the three repeated header rows ("Start-up costs" / agency line / column headers — bp/13's four long tables each mark 3 leading rows `w:tblHeader`); the "constant −14pt" band offset was the absent header block. Cause: `PlaceSplitRow` re-emitted headers only on continuation fragments (`!isFirstFragment`), but the sliver advance before a FIRST fragment is a page break too — and it is exactly the path the kept row-level fit trigger routes boundary rows through, bypassing the `EnsureSpaceFor` path that re-emits. So the trigger had been silently stripping repeated headers, and the loss rode into the earlier adjudication disguised as geometry movement. Fixed by re-emitting on `!isFirstFragment || advanced`. Adjudication: **14 pages better, 1 same, 0 worse** (bp/13 p14/p16/p20 and `header_row_repeat/01` p2/p3, all three backends; aggregate 6.579 → 6.333); page counts 325/325; baselines regenerated. Residual on those pages is now −8..−11pt — the repeated header block itself renders slightly SHORTER than Word's, a smaller, separate question (likely the header rows' heights, worth its own measurement if the pages are revisited).

  **Two genuine divergences remain open, both now cleanly measured:** (1) the engine absorbs the trailing empty-PARAGRAPH row Word carries (AFTER at 75.36 vs 174.72) — unpicking that touches the absorption rule that stops a letter template's spacer rows from starting a page; (2) `FinishPage` drops the natural-overflow blank page Word renders (`_probe_trail2_flowblank`: 54 exact lines + a trailing empty paragraph = 2 pages in Word, page 2 genuinely blank; the engine gives 1, and the `FinishPage` comment's claim that "Word does not render" such a page is refuted). Both are corpus-motivated rules, so each needs its own adjudication.

  The row-level half of the trigger IS kept — a row that does not fit the space left splits, exact rows excluded. It cannot reach `fit_off` while the table-level routing stays reverted.

  **PARKED 2026-08-07 (deliberate).** The table-level widening chases a synthetic probe, not a corpus deficit: page counts are 325/325/325 without it, no corpus document is known to need it, and both attempts found the corpus pushing back. The trajectory is convergent (129 bad pages → 3 documents → 1 document with a named mechanism), so a future retry starts from the `resumes/06` blocker with all evidence in this entry. Retry only when a real document needs the behaviour.

  Widening the trigger (route a table that will not fit the space left to the row-by-row path, and split any row that will not fit) reproduced Word's *shape* on the probe — both unflagged fixtures split — but **wrecked the corpus: 129 pages moved, 87 worse against only 38 better, aggregate mean |Word−render| 10.375 → 13.299, individual pages up to +20.7 (business-plans/15 p13/p15/p11)**. Reverted.

  **The last-line rule was probed next (`_probe_lastline_*`, 2026-08-06) and it is NOT simply one path being wrong — both readings are refuted.** Sixty paragraphs against a 648pt band, in the flow and in a splitting one-row table, at exact and at auto line spacing:

  | | Word flow | engine flow | Word cell | engine cell |
  |---|---|---|---|---|
  | 13pt EXACT | 49 | **50** | 49 | 49 |
  | auto | 42 | 42 | 41 | 41 |

  So `PlaceParagraph`'s ascent test is too generous under exact spacing — auto-spaced text cannot tell the two apart, which is why it survived — but **switching it to the full line box regressed `image_wrap_square` from 2 pages to 3, losing Word's count**, so the box test is too strict for that document. Attempted and reverted; the scoreboard is back to 325/325.

  **The ink-box rule was then built and tried (2026-08-06) — it matches Word on ALL FOUR controlled fixtures and STILL costs `image_wrap_square` its page count, so the last-line rule is not the whole story.** `FontMetrics` gained `BaselineDescentUnits` (the `usWinDescent` partner to `BaselineAscentUnits`), `LaidOutLine` an `InkHeight` = ascent + descent clamped to the line box, and `PlaceParagraph` tested that instead of the ascent. Result: flow 49 / cell 49 / auto-flow 42 / auto-cell 41 — every fixture matching Word, where ascent-only gets the exact cases wrong and the full box is too strict. But the corpus adjudication came back **byte-identical to the full-box attempt** (same pages, same 14.610 → 14.511), and `image_wrap_square` went 2 pages to 3 again. Reverted.

  The identical numbers explain themselves: **for a font with no external leading the ink box IS the line box**, so the two rules only differ where `lineGap` is non-zero. The corpus's fonts are mostly zero-gap, which is why the ink rule bought nothing there.

  **The line-height suspicion was then measured and is REFUTED (2026-08-07) — the engine's line height is Word's, to 0.002pt.** `_probe_lastline_auto_flow` is an exact ruler for this, because every line reads `L01`…`L60` in the same glyphs, so ink tops carry no content noise: across 40 gaps Word spans 617.760pt, a pitch of **15.4440pt**, against the 15.4419pt the metrics predict for Calibri 11pt at `w:line="276"` (the 2500-unit box — `usWinAscent` 1950 + `usWinDescent` 550 — times 1.15). Total error over 40 lines is 0.08pt, a fifth of a pixel. A 15.36pt pitch, which the raw `image_wrap_square` bands appear to show, is refuted by 3.36pt (7 pixels) across the same span. That apparent 15.36-vs-15.48 gap was an artifact of comparing a 150dpi Word baseline against a 200dpi render: at matched dpi the two agree pitch-for-pitch, and the engine's column merely sits ~1px higher overall.

  So the fit rule is back to being the whole question, and the three Word measurements now available are jointly decisive — against the engine's traced boxes, content bottom 720.00 in every case:

  | Word's own render | verdict | box bottom | ink bottom | ascent bottom |
  |---|---|---|---|---|
  | exact flow, line 50 | **rejects** | 722.00 | 722.43 | 719.47 |
  | auto flow, line 42 | **keeps** | 720.56 | 718.55 | 715.59 |
  | `image_wrap_square` col, line 6 | **keeps** | 724.36 | 722.35 | 719.39 |

  Every single-quantity reading dies here: the full box on rows 2–3, the ink box on row 3, the ascent on row 1. No threshold can separate rows 1 and 3 either — they straddle by 0.08pt in ascent bottom while disagreeing in outcome — and the box overhang is non-monotone across the three (0.56 kept, 2.00 rejected, 4.36 kept). **The discriminator is `w:lineRule` itself: under `exact` Word requires the whole declared box, under `auto` only the ascent.** That reading fits all three, and matches what "exact" means — the declared height is an absolute reservation. It also predicts the overhang is drawn and clipped, which Word's render confirms: `image_wrap_square`'s line 6 has a full-width ink band 715.20–720.00 that stops dead at the content bottom, where line 5 above it trails descenders 1.92pt past its own band.

  **LANDED 2026-08-07.** `PlaceParagraph` reserves the whole line box unless the rule is `Auto`, in which case it reserves the ascent; the cell splitter is untouched, since its full-height test already matched Word on both cell fixtures.

  **`atLeast` was assumed to follow `auto` and does NOT — probed after the exact half landed, and the assumption was wrong three ways** (`_probe_lastline_atleast_a`/`b`/`c`). Word keeps 41 lines at a declared 15.5pt where the lenient reading takes 42, and 30 at 21pt against 31. The decisive fixture is `c`: a declared 10pt that LOSES to Calibri's natural 13.4277pt pitch, where Word keeps 44 against 45 — strict even though the box is then the font's own and identical to what single `auto` produces. So the leniency belongs to the `auto` rule itself, not to the box's provenance, and the test is `rule != Auto` rather than any declared-versus-natural comparison.

  All **seven** probes now agree with Word (49 / 49 / 42 / 41 / 41 / 30 / 44 — the exact-flow case moved 50 → 49, which is what the two reverted readings also bought, except that this one leaves auto-spaced text alone). The corpus is **unmoved**: page counts hold 325/325/325 and the full suite passes 3040/3040 with zero baseline diffs, because none of the 12 `lineRule="exact"` documents (`complex_spacing`, `line_spacing_exactly`, `resumes/11`, `newsletters/08`, `cover-letters/01`/`02`/`04`/`08`/`16`, `brochures/03`, `agendas-minutes/13`/`16`) nor the 9 `atLeast` ones (`complex_spacing`, `line_spacing_at_least`, `nonstandard_main_part_name`, `resumes/06`/`11`/`19`, `newsletters/05`, `cover-letters/06`, `business-plans/12`) happens to break a paragraph inside the narrow window where the readings disagree. The rule is therefore pinned by `CanonicalFragmenterTests` — `An_exact_spaced_line_needs_its_whole_box_inside_the_margin`, `An_auto_spaced_line_keeps_only_its_baseline_inside_the_margin` and `An_atLeast_spaced_line_needs_its_whole_box_inside_the_margin` (parameterized over the binding and font-wins cases), each guarding that its discriminating line really does straddle the margin — exactly as the widow/orphan rules are.

  That unblocks the rest of #25: the fit rule is settled, so the split-trigger widening can be retried against a correct last-line test.

  Note also that Word's flow and cell genuinely differ under auto spacing (42 vs 41, which the engine reproduces) — a separate, uncharacterised difference in a cell's usable height.

  `w:cantSplit` itself is read and honoured, and the widow/orphan half of this entry is closed (both paths settle the two rules in Word's order).
...
- FIXED 2026-08-05 (structural anchor resolution, fourth attempt — the Word probe settled it): `_probe_anchor_ref` pinned Word's paragraph-anchor reference to the paragraph's PRE-SPACING top (an offset-0 anchored shape on a 60pt-before paragraph sits at the previous paragraph's bottom edge; the zero-before control coincides with its line top). The landing design: the parser wires every float parsed by a ParseParagraph call to that call's FIRST paragraph fragment (structural — the drawing's owner paragraph, no ordering heuristic); the Fragmenter records each paragraph's pre-spacing top and resolves BACKWARD-only (dictionary hit for flush-first orders like this document's background-shape branch; the forward case emits at the cursor, provably the same value, preserving old behaviour byte-for-byte). Adjudication: agendas-minutes/05 32.3 → 5.0 (production tie), menus/05 23.2 → 15.3 (beats production), wedding/07 / labels/07 / cards/03 (the heuristic attempt's casualties) all at exact production parity. Corpus: 4 backend-scenarios moved, ALL improved (+0.029 agendas ×2, +0.002 brochures/07 ×2), zero regressions, zero page-count changes.
...
- FIXED 2026-08-06 (the last named flip-gate residue). **The long-standing "header reservation" attribution was wrong** — instrumenting `HeaderReservedTop` showed the header is NULL on page 1: the section sets `w:titlePg` with only a default header, so Word shows no first-page header and `contentTop` is just the 18pt margin. The reservation code never runs. The real cause: the body's three-row letterhead table has every row `w:hRule="exact"` (22.3 + 25.9 + 66.2 = 114.4pt), and Word starts "Memorandum" at exactly margin + 114.4pt, but `TableHeightCalculator`'s border-collapse pass grew those rows by their collapsed horizontal edges (outer top + outer bottom + two interior, ~0.75pt each = 3.05pt). ECMA-376 §17.4.81 makes an exact height verbatim and Word honours it against borders exactly as against overflowing content — it draws the edge and clips. Exact rows are now exempt from that pass. Body lines went from +4..+6px below Word to −2..0px; full-resolution mean |Word−render| improved on all three backends (skia 6.55→4.75, PDF 7.08→5.10, ImageSharp 7.05→6.66). NOTE for future digs: the coarse band adjudicator reports this scenario as WORSE (16 vs 17 bands breaks its index pairing) — it is the documented segmentation artifact, and full-resolution pixel comparison is the only reliable verdict here.
...
- FIXED 2026-08-05 (both root causes; the flip's worst breach −0.107 became +0.11..+0.12): (1) the canonical mark fallback ignored `ParagraphMarkFontSizePoints` when the mark had no rPr and no runs — cards/05's header holds an empty 2pt-mark paragraph, so the engine reserved a full 11pt header line and started every card table 11pt low; (2) the band-float resolver admitted only behind-text shapes, dropping the two front-text 0.5pt fold-guide rules. With both fixed the engine matches production (and Word) at delta 0 / mean 0.06 grey levels. The earlier "saturation" lead was a sampling artifact of comparing shifted renders — retracted. Same fixes lifted cover-letters/03 +0.09, resumes/09 +0.05, menus/06, resumes/19.
