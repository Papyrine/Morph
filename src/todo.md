...
- **#43 Morph's Calibri renders 2.37% narrow at 12pt, which blocks the default-font family fix** (found 2026-08-13). Two separate findings, the second gated on the first:
  ...
  - **Switching the family alone makes the corpus worse**, so it is not a one-line fix. Measured over the 150 scenarios it moves: mean AE 0.04136 → 0.04151, and four scenarios lose Word's page count (`explicit_break_blank_page`, `html_complex`, `business-plans/09`, `newsletters/07`) against the documented 325/325 invariant. Tables and short text improve sharply (`complex_document` −0.048, `table_default_style` −0.037, `table_cell_padding` −0.016) while flowing text regresses (`complex_spacing` +0.102, `multiple_pages` +0.031, `two_columns` +0.007), because the metric error compounds over many lines.
