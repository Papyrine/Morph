...
- **#43 Word's Calibri advance model is SOLVED and TOOLED (2026-08-18); activating it — and the Calibri-12 default-font flip it gates — is blocked on KERNING.** The 2026-08-13 characterisation ("Word departs from hmtx by grid-fitting… NOT cheaply fixable… entry point is hinted advances") is superseded: the model was measured from Word's own XPS (per-glyph `Indices` advances — see the probes section in `CLAUDE.md`) and no hinting execution is needed.
  ...
  - **Why parked: kerning.** With sidecars active the corpus measured 5 pages better / 29 worse, mean AE +0.0016, and `resumes/16` (uppercase-heavy headings) lost Word's page count (1→2). On `table_colors`' title line the sidecar model is +0.35% of Word against the linear track's −2.26% — but the residual is Word's KERNING (its `Ta` renders 8px against 10px unkerned), which Morph does not model, and which the linear track's narrowness had been cancelling. Landing needs GPOS pair kerning (gated per run by `w:kern`), then reactivation and re-adjudication. Word kerns even style-less documents.
