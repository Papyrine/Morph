...
- **#45 Spreadsheet geometry: the column-width and fit-scale RULES are both known and neither can land, because the fit scale's INPUTS are wrong by far more than either** (probed at A4, 2026-08-14).

  **Rule 1 — column width.** A declared `col/@width` becomes `width * maxDigitAdvancePx`: the EXACT advance, no rounding, no additive padding. Six faces, read off printed gridlines (`_probe_grid_*`, columns of 6/30/60 characters): Excel's unit is 7.519 for Calibri 11, 7.370 for Arial 10, 7.685 for Corbel 11, 8.963 for Georgia 11, 8.000 for Segoe UI 11, 8.167 for Consolas 11 — against max-digit advances of 7.434/7.415/7.691/9.002/7.906/8.064, i.e. ratios 0.994 to 1.020. It is the MAXIMUM digit and not `'0'`, which only shows on a face with proportional figures: of the six, only Corbel has a wider digit than its `'0'`, and the maximum moves its prediction from 2.0% wide to 0.1%. The 5px padding is already inside the stored width (ECMA-376 §18.3.1.13 writes it as `(chars*MDW+5)/MDW`), so `ToPoints`'s `+5` double-counts it; fitted padding measures −0.22 to +0.22px.

  **Rule 2 — the fit scale is floored to a WHOLE PERCENT.** Probed as a pure ratio so the width unit cancels (`_probe_fit_*` against `_probe_nofit_*`, the same fixture with and without `fitToPage`, scale read as one column's fitted width over its unfitted width): grids needing 93.06 / 86.79 / 81.45 / 74.42 / 68.56 / 62.04 percent render at 92.21 / 85.99 / 81.20 / 74.01 / 68.07 / 61.09. `ResolveScale` uses the continuous ratio. Solving the six constraints also puts Excel's printable width at 973.6-978.4px against the 978.5 that page-minus-margins gives, so `ContentWidth` may be a couple of px generous.

  **Why neither lands.** Implemented together — exact max-digit advance, no padding, `SheetGeometry` taking the same unit instead of its hardcoded 7, and `Math.Floor(scale * 100) / 100` — the corpus comes out SSIM −0.4695 / AE −0.1039 over 110 pages, 43 pages better and 54 worse, no page-count change. Width alone was SSIM −0.4525 / AE +0.2132. The crops agree with the metric here, unlike the placement fixes above: `home-contents-inventory-list` was already near-exact (grid 1043x339px against Excel's 1041x339) and goes to 1049x343.

  **The dominant error is elsewhere, and this is where to start.** 37 of the 40 workbooks set `fitToPage`, so the scale governs every length in the sheet, row heights included. Comparing our computed scale against the one Excel CACHES in `pageSetup/@scale` across the 30 fitted sheets that declare it gives a mean absolute difference of **9.5 percentage points**, with `wedding-budget-template` at 31 against Excel's 63 and five more past 20 — an order of magnitude more than either rule above is worth. Mostly we scale too SMALL, i.e. our natural size is too big. That comparison is crude and must be confirmed by instrumenting the parser rather than approximating it: it read the `dimension` range where `ResolveRange` honours `Print_Area`, and assumed the width axis binds where `fitToHeight` may. Those two are exactly the first suspects, along with `NaturalHeightPoints` ignoring `autoRowHeightFactor` so an auto-height sheet under-reports its natural height. Settle the scale first; the two rules above are then cheap to land on top, and are already measured.

  **Method note.** `RenderHelper` sets `PaperSize = xlPaperA4` over COM and reads it back, and Excel REPORTS A4 while a Letter-paper driver silently exports Letter — the guard passes and the A4 layout is squeezed onto Letter, scaling every measurement by ~0.92. That produced a whole earlier round of conclusions that were simply wrong (a phantom "Excel's MDW is 0.92 of the font advance by an unexplainable per-font factor"). **Verify the paper by rendering a fixture and checking the page is 1123x794, not with `Get-PrintConfiguration`** — that cmdlet reported Letter on this machine while Excel was exporting A4.

  **Until this closes, SSIM misreads spreadsheet text-placement fixes.** Both cell-overflow changes of 2026-08-14 scored NEGATIVE while being visibly correct against `expected_0001.png`, because correct placement stopped masking the geometry error. Read the crops, per `docs/fidelity-audit.md`.
