...
- PDF-flip-gate root causes, refined 2026-08-04 (page-7 referee measurement, Word expected vs both PDFs): (1) POSITION — production matches Word's card-block box exactly (ink rows 637–1164 at 150dpi); the ENGINE draws the whole block 74px (~35pt) LOWER — NARROWED: TableHeightCalculator prints identical row heights under both measurers ([273.60, 273.60/280.80]), so the ~35pt sits ABOVE the table — the pre-table empty paragraph or the page-top spacing rule on explicit-page-break pages (production suppresses spacing-before there via ShouldSuppressPageTopSpacingBefore, compat-15; check the Fragmenter's equivalent). Cards are inline 360pt JPEGs in a 2-row 3-column table. (2) COLOUR — Word renders the picture most muted (centre RGB spread 20), production close (33), the engine far too vivid (64): the engine PDF embeds the raw JPEG bytes (PdfPainter → context.GetImage) while production's decode path lands near Word — check ICC/colour handling in the embed. The fold line lead is retracted — all three renders have it (thumbnail artifact).
