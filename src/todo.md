...
- MAJOR | all | p1 | header banner table clipped to 962px of the 1240px page width: the header's table is `tblW=12508 dxa` with a NEGATIVE `tblInd=-1593 dxa`, i.e. deliberately wider than the text column and bled to both page edges. Word renders it edge to edge (ink spans x=0..1240); Morph confines it to the text column, so the bar stops ~78% across and the right quarter of the page is bare.
- MAJOR | all | p1 | body content sits 54-59px (~2 line heights) higher than Word and the H1 title overlaps the banner: Word clears the banner and starts "AUTUMN 2022 Bid Advice Summary" at y=218, Morph at y=~140 (its ink merges with the banner band). Downstream, "PORTFOLIO A" y=221 vs Word 280, "SMITH, John" 282 vs 339, "Notes:" 427 vs 481. The first-page header's banner height is not being reserved against the body.
- MEDIUM | all | p1 | "Notes:" box 220px tall vs Word's 238 (~8% short). Its single cell contains only seven `<w:br/>` runs, so the height is entirely break-only line boxes — Morph measures each ~2.5px short.
...
- MINOR | all | p1 | page renders 1240x1754 vs Word's 1240x1753. `w:pgSz` declares `w:h=16840` (842.0pt) but also `w:code="9"` — the A4 printer paper code — and Word honours the code's true A4 height (841.89pt) over the rounded twips. Morph converts the twips arithmetically. **This is the only page-size mismatch in the corpus, and it costs the scenario its SSIM entirely** (`PageComparison` returns null SSIM when the dimensions differ), leaving AE as the sole metric here. Fixing `@code` would restore it. The other six A4 scenarios all declare `w:h=16838` and match Word exactly.
