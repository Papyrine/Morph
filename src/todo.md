...
- **#45 A spreadsheet's COLUMNS are 2-14% wider than Excel's, and the rule behind Excel's width is still unidentified** (measured 2026-08-14 with `_probe_colwidth_*` / `_probe_grid_*` / `_probe_zero_*` / `_probe_size_*`). Excel's printed width for a declared `col/@width` is LINEAR in that width with **no additive padding**, against Morph's `Truncate(chars * MDW + 5)` with MDW rounded to a whole pixel. Read two independent ways that agree within 1px — marker-glyph offsets, and `printOptions/@gridLines` so the boundaries are drawn rules — on a Letter-landscape page at 96 DPI whose grid starts at exactly x=72 (0.75in), so the geometry is unscaled and these are true pixels. Width 60 declared:

  | font | Excel px | Morph px | error | Excel MDW | font `'0'` advance |
  | --- | --- | --- | --- | --- | --- |
  | Calibri 11 | 415 | 425 | +2.4% | 6.907 | 7.434 |
  | Arial 10 | 406 | 425 | +4.7% | 6.759 | 7.415 |
  | Corbel 11 | 424 | 485 | +14.4% | 7.056 | 7.534 |
  | Georgia 11 | 495 | 545 | +10.1% | 8.241 | 9.002 |
  | Segoe UI 11 | 442 | 485 | +9.7% | 7.370 | 7.906 |
  | Consolas 11 | 450 | 485 | +7.8% | 7.500 | 8.064 |

  **What the probes settled.** (1) The additive padding is ~0, not 5 — fitted at 0.44/0.56/0.67/0.56/−0.22/0.00 px. ECMA-376 §18.3.1.13 agrees: the stored width already contains the 5px, since it is written as `(chars*MDW+5)/MDW`, so our `+5` is a double count. (2) Excel's MDW is never an integer, so rounding it is wrong; the worst errors are exactly the fonts we round UP (Corbel 7.534→8, Segoe UI 7.906→8). (3) Excel's column unit tracks its own RENDERED digit advance (6.867 measured against a 6.907 column unit for Calibri, isolated as `(w60−w30)/30` so side bearings cancel), and both sit ~8% under the font file. (4) Installed and `src/Fonts` metrics are identical, so the font files are not the variable. (5) **It is NOT the hinting dead-end that blocks #43**: Calibri probed at 6/11/22/44pt gives MDW/pt of 0.6142/0.6279/0.6212/0.6206 — constant across a 7x range, so the unit is a plain linear metric, not per-ppem grid fitting.

  **Why nothing was changed.** The two known-wrong parts partially CANCEL, so fixing them without the rule is a regression: dropping the `+5` and using the exact advance moves the mean absolute error only 8.2% → 7.9% while taking Calibri from +2.4% to +7.5% and Arial from +4.7% to +9.6% — the two commonest corpus fonts, made twice as wrong. What is missing is the source of the per-font factor, which is 0.909-0.937 of the `'0'` advance and NOT constant enough to fit. Candidates not yet tested: a different glyph or figure set standing in for "maximum digit width", and a GDI cell-height vs em-height mismatch in how Excel creates the measuring font (checked against `usWinAscent+usWinDescent` and rejected: predicts 0.880 for Georgia against a measured 0.915). Do not ship a fitted constant — that is the failure mode #42 exists to name.

  **Until it closes, SSIM misreads spreadsheet text-placement fixes.** Both cell-overflow changes of 2026-08-14 scored NEGATIVE while being visibly correct against `expected_0001.png` — single-line at −0.00005 aggregate and the clip at −0.0013 — because correct placement stopped masking this. `to-do-list-for-projects` p2 is the live example after the centring fix (−0.0094): its grid is narrow, so its slack is too big and centring pushes it PAST Excel. Read the crops, per `docs/fidelity-audit.md`; do not revert a spreadsheet placement change on the number alone.
