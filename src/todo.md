...
- MAJOR | skia | p1,p2,p4,p5 | **REGRESSION, found by the 2026-07-20 re-audit.** These four skia baselines are 100% SOLID NAVY — 3 unique colours, zero content, 18-30KB PNGs against ImageSharp's 400KB. A broken render was promoted into the baselines and the SSIM suite has been green against it ever since (it compares to the baseline, not to Word). `git log` on the baseline dates it to `8da1f624d` (2026-07-18, "Assemble connector-line arrow glyphs cleanly"), where the file dropped 318,837 → 18,851 bytes; it predates all 2026-07-20 work. p3/p6 render normally, and ImageSharp/PDF render content on all pages (with a page-wide navy arc over the masthead that may share the root cause). A corpus-wide scan for the same signature found NO other affected scenario (`explicit_break_blank_page` p2 is a legitimately blank page). Fix this before trusting any other finding in this scenario — several below could not be judged in skia because there was nothing to look at. **These four pages are allow-listed in `BaselineHealthTests.KnownDegenerate` (the promotion-time guard added 2026-07-21) so the suite stays green; when this is fixed and the baselines regenerated, delete those four lines from the allow-list — the guard's reverse assertion will otherwise fail because the pages are no longer degenerate.**
