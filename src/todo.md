...
- **#25 `PlaceParagraph`'s widow/orphan rule can still leave a lone line, and `w:cantSplit` is unread** (2026-08-06). Word settles the two rules in ORDER, letting the second act on the first's result: the widow carry pulls a second line down to join a lone line at the next page's top, and if that leaves a lone line behind, the orphan rule then moves the whole paragraph. `PlaceParagraph` treats them as mutually exclusive `else if` branches, so `fit = remaining - 2` can land on 1 and emit exactly the orphan the rule exists to prevent — a three-line paragraph with room for two ends up 1/2 where Word gives 0/3. The cell splitter got the ordered form when row splitting landed (proven on `business-plans/15` page 16: 44 ink bands → 43, matching Word), so the two paths now differ. Aligning the flow path is a corpus-wide change and needs its own gate — it was deliberately not folded into the splitter's measurement. Also still open: `w:cantSplit` selects move-whole for a row that would otherwise split; it is implementable now that rows split, but the parser does not read it, and no corpus document sets it.
