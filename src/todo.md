...
5. **Floating/anchored decorative art missing or misplaced** — the largest source of MAJOR findings; ten fix passes landed 2026-07-19 (cell-float hoisting/cell-attached rendering, relativeHeight z-sort, nested-transform affine composition, document-order interleave, a:grpFill resolution, group-frame clipping, pic:spPr geometry crops, walk authority over non-identity nesting). Architecture, authority rules and the attempted-and-reverted decision log now live in `docs/floating-art-pipeline.md` — the history was moved there. **Still open:** (a) brochures/07 picture-size class — cell-anchored pictures render at ~half height over the quote text; (b) brochures/03 — the right circle photo renders in colour where Word greyscales it (greyscale is not declared in the pic XML — likely a theme/effect route, #8 class) and sits ~20pt high; (c) labels/16 — bears sit ~30px low (header-float vertical resolution); (d) remaining missing freeform/vector shapes: brochures/04/06 (chevrons, balloon art, quote box), business/04/05 (banners, watercolor blobs), business-plans/01 (accent bar), cover-letters/06/07/10 (banner bands, logos), letters/02/03/11 (frames, gradient banners, logo strip), cards/18/12/05/06 (fold guides), labels/02/03 (cell borders, tear lines — labels/03's 90-degree tear lines are rotated line connectors, dropped by the rotated-line bail), menus/01/04/06 (floral art, doodle pattern, red bars), newsletters/13/14, resumes/10; (e) remaining missing/mangled pictures: brochures/07 (photos swapped/missing/misplaced), newsletters/03/04 (inset photos), newsletters/08/11 (photos clipped to slivers), cards/02 (blossom behind card), business-plans/12 (SWOT graphic), cover-letters/09 (profile photo), newsletters/13/14.
6. **Shape geometry defects** — preset polygons, text-box chrome, picture flips, line alpha and connector assembly are resolved (`docs/word-features.md`). **Still open:** (a) outline-only shapes (`a:noFill` + `a:ln`) are not emitted — two attempts reverted; blockers and re-landing prerequisites recorded in `docs/floating-art-pipeline.md` (decision log #6); (b) business-plans/02 arrow construction; (c) duplicated art (cards/06, business/06); (d) group-child offset (menus/07 / inline_group_crop); (e) stray art Word hides in cards/04-class scenarios is resolved by group-frame clipping, but labels/16-class strays from other subsystems may remain.
...
11. **Numbering defects** — bullet glyph coverage, cross-table restarts and PDF markers resolved (`docs/word-features.md`). **Still open:** SWOT bullets lose colours (business-plans/12); labels/16 picture-bullet-class icons sit ~30px low (reclassified: header-float vertical resolution, see systemic #5c).
12. **TOC rendering broken** — ✅ resolved — tab-stop clamp + Hyperlink-style suppression (`docs/word-features.md`, Tab Stops / TOC rows; the vacuous-fixture lesson is in `docs/floating-art-pipeline.md` decision log #5). **Still open:** page numbers are the cached values (live PAGEREF needs a bookmark→page map); numbers sit ~4pt left of Word (clamp lands at the cell content edge, Word spills into padding).
