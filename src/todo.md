...
- **#43 Morph's Calibri renders 2.37% narrow at 12pt, which blocks the default-font family fix** (found 2026-08-13). Two separate findings, the second gated on the first:
  - **Word's built-in default font is CALIBRI 12pt, not Aptos.** Probed with a bare three-part package, one paragraph, no `w:rFonts` anywhere: Word draws the sample 632x21px at 150 DPI, matching its own explicit Calibri 12 exactly on both axes (Aptos 12 is 665x20, Times 12 is 629x23). Morph uses Aptos, so every style-less document is laid out ~4.1% wide — which is what makes `table_borders` wrap "Cell (1,1)" where Word keeps it on one line. The comment on `builtInDefaultFontSizePoints` claiming `long_paragraph` "solves to 12pt Aptos" is wrong about the FAMILY (the 12pt size is right): Morph+Aptos matches that document's 27 lines only by coincidence, a +4% family error landing on the same wrap points as the −2.4% metric error below.
  - **Switching the family alone leaves the corpus marginally worse**, so it is not a one-line fix. Measured over the 150 scenarios it moves: mean AE 0.04142 → 0.04158. Tables and short text improve sharply (`complex_document` −0.048, `table_default_style` −0.037, `table_cell_padding` −0.016) while flowing text regresses (`complex_spacing` +0.102, `multiple_pages` +0.031, `two_columns` +0.007), because the advance error compounds over many lines.
    - **CORRECTION (2026-08-13):** this entry previously claimed four scenarios lost Word's page count under Calibri. That was wrong. It counted Verify's `.received.` files, which exist only for the pages that DIFFER — so "18 expected, 4 received" meant four pages changed, not a four-page document. Re-measured from `ResultingPageCount` in each `*_result.received.json`: **zero** scenarios change page count. Use the JSON, never the received-file count, when judging pagination.
  - **The blocker, now characterised (2026-08-13) and NOT cheaply fixable.** Morph's Calibri metrics are CORRECT — they are the font file's. It is Word that departs from `hmtx`, by grid-fitting advances at some ppem and not others. Swept Calibri and Aptos at 8/9/10/11/12/14/16/20/24pt, one size per page (36 and 48pt wrap and were discarded): Calibri is within ±0.6% at 8, 9, 11, 14, 20 and 24pt but off by −3.6%, −2.4% and −1.3% at 10, 12 and 16pt, while Aptos holds ±1.2% everywhere. Glyph HEIGHTS match Word exactly at every size (18/18, 21/21, 35/35 px), so the point size is right and only horizontal advance differs.
    - Isolating one glyph — 30 vs 60 `n`s, so `(w60−w30)/30` is the exact advance and side bearings cancel — Word's Calibri `n` is 12.033px at 11pt and 17.500px at 16pt against the font's 12.040 and 17.513 (both exact), but 11.267px at 10pt and 13.733px at 12pt against 10.946 and 13.135 (+2.9%, +4.6%). Non-integer averages rule out integer-pixel advance rounding; both installed Calibri versions (6.27, matching `src/Fonts`, and a stale 2.10) declare `n` at 1076 and 1064 units, neither of which is the ~1126 units Word implies at 12pt; neither file has an `hdmx` table.
...
- **#45 Spreadsheet geometry: three of the four inputs are now MEASURED and implementable; the row-height model is the one still missing, and nothing lands without it** (probed at A4, 2026-08-14). Everything below was implemented together and reverted — the branch is reconstructable from this entry, and each rule is independently evidenced.

  **Rule 1 — column width = `width * maxDigitAdvancePx`.** Exact advance, no rounding, no additive padding. Six faces read off printed gridlines (`_probe_grid_*`, columns of 6/30/60 characters): Excel's unit is 7.519 Calibri 11, 7.370 Arial 10, 7.685 Corbel 11, 8.963 Georgia 11, 8.000 Segoe UI 11, 8.167 Consolas 11, against max-digit advances of 7.434/7.415/7.691/9.002/7.906/8.064 — ratios 0.994 to 1.020. It is the MAXIMUM digit, not `'0'`: only Corbel of the six has a wider digit, and the maximum moves its prediction from 2.0% wide to 0.1%. The `+5` padding is already inside the stored width (ECMA-376 §18.3.1.13 writes it `(chars*MDW+5)/MDW`); fitted padding measures −0.22 to +0.22px. `SheetGeometry` must take the same unit — it hardcodes 7 while the builder measures the face, so anchored art detaches on any non-Calibri-like workbook.

  **Rule 2 — the fit scale is floored to a WHOLE PERCENT.** Probed as a pure ratio so the width unit cancels (`_probe_fit_*` against `_probe_nofit_*`, same fixture with and without `fitToPage`): grids needing 93.06/86.79/81.45/74.42/68.56/62.04 percent render at 92.21/85.99/81.20/74.01/68.07/61.09. `ResolveScale` uses the continuous ratio.

  **Rule 3 — `dimension` is a hint, not a measurement.** Excel writes the range it last tracked and prints neither the stale columns nor the stale rows. Intersecting it with the cells the sheet actually carries (a cell with no value still counts — its formatting prints) is what Excel's used range amounts to. `to-do-list-for-projects` declares A1:K and stops at F; `construction-proposal` declares P and stops at J. That stale tail inflates `NaturalWidthPoints` and so shrinks the fit scale: the first came out at 63% against Excel's 90%.

  **What is still missing: the row-height model.** With all three applied the corpus reaches SSIM −0.3157 / AE −0.2254 over 110 pages, 42 better and 60 worse — up from −0.4525/+0.2132 for Rule 1 alone, so every rule pulls the right way and none is sufficient. The residual is legible in the extents: WIDTHS improve and HEIGHTS get worse. `simple-basic-pink-blue-timesheet` goes 659→649px wide against Excel's 614 while its height goes 936→953 against 933; `weekly-lesson-planner` 648→691 against 640. A narrower natural width means less shrink, and the larger scale multiplies the ROW heights with it, so a row model that was calibrated against the old width now overshoots. `autoRowHeightFactor = 1.31` is the constant to re-probe (and note `NaturalHeightPoints` ignores it entirely, so an auto-height sheet under-reports its natural height — harmless today only because the height axis binds on ZERO of the 30 fitted corpus sheets, measured).

  **Instrumented ground truth.** `ResolveScale` dumped against the scale Excel caches in `pageSetup/@scale` over the 30 fitted sheets: mean absolute difference **5.5 points** before any fix (an earlier hand-rolled estimate said 9.5 — it read `dimension` where `ResolveRange` honours `Print_Area`; use the instrumented figure). Worst were `wedding-budget-template` 32 against 63 and `to-do-list-for-projects` 63 against 90. Treat the cached scale as a strong hint rather than truth: Excel writes its last computed value and it can be stale.

  **Method note — the trap that produced a whole round of wrong conclusions.** `RenderHelper` sets `PaperSize = xlPaperA4` over COM and reads it back; Excel REPORTS A4 while a Letter-paper driver silently exports Letter, so the guard passes and the A4 layout is squeezed onto Letter, scaling every measurement by ~0.92. **Verify the paper by rendering a fixture and checking the page is 1123x794 — not with `Get-PrintConfiguration`**, which reported Letter on this machine while Excel was exporting A4.

  **Until this closes, SSIM misreads spreadsheet placement fixes.** Both cell-overflow changes of 2026-08-14 scored NEGATIVE while visibly correct against `expected_0001.png`. And it cuts both ways here: `weekly-lesson-planner` scored +0.12 on a change that made its geometry measurably WORSE. Read the crops and the extents, per `docs/fidelity-audit.md`; on this subsystem neither metric is trustworthy alone.
...
- **#26 (HTML) White/light text invisible — CLOSED 2026-08-07, and the recorded cause was WRONG.** The entry blamed the export for painting neither dark page backgrounds nor dark shape fills. It painted both: `WriteShape` already emitted background shapes as absolutely-positioned `<svg>` at `z-index: -1`, and agendas-minutes/02's navy panel rendered correctly all along. The text was invisible because it had been PUSHED OFF the panel — the decorative wave banners above it were wrap-none floating images emitted in flow, so their height displaced the masthead and roster past the 792pt panel and onto white page. Fixing the float placement (#25 above) fixed the text: agendas-minutes/02's masthead, date block and all 8 roster entries, newsletters/10's title, brochures/03's white-on-navy body all read correctly now. LESSON: an "invisible text" symptom is not evidence that the background is missing — check where the text actually landed before blaming the paint.
