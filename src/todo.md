...
- MEDIUM | all | p1 | Cover renders a footer ("SAND + POLISH CONTRACTORS" + page number "2") that Word does not show. **Not suppression** — `w:titlePg` names `footer3.xml`, which genuinely holds that text and a PAGE field, and Word lays it out. The cover's grey `wps` rectangle is a behind-text BODY float spanning 575.4→792pt and the footer sits at ~756pt, inside it: Word paints body floats above the header/footer story, so the footer is buried. Morph paints it on top. Only ~1% of p1's error, but business-plans/15's "footer bar on the TOC pages" is probably the same rule. **Needs a probe** (behind-text rectangle over a footer, one Word render) before anything is coded to it.
- MEDIUM | all | p1 | Cover title and subtitle sit ~24pt HIGH (the entry here previously said "lower", which the pixels refute: Word's title baseline is at 637.9pt, Morph's at 622.4). The cover is 22 filler paragraphs before the Title and **Morph parses 21** — the paragraph carrying both cover anchors is discarded, because `DocumentParser`'s `onlyFloatingShapes` predicate (`result.All(_ => _ is FloatingShapeElement)`) does not recognise a `FloatingImageElement`, so a paragraph that lifted out one image *and* one shape matches no branch and loses its mark. The comment above that line states the intended rule correctly; only the type test is too narrow. Restoring the line puts the SUBTITLE within 0.2pt of Word (688.14 predicted vs 688.3 measured); the TITLE would still be 8.5pt low, because Word places the baseline higher inside a `w:line="192" lineRule="auto"` (0.8×) compressed box than Morph does — **needs its own probe** (one fixture, a column of large-font paragraphs at several `w:line` multiples, baseline-to-baseline measured). Widening the predicate alone measured **−0.53 aggregate over 32 other scenarios (27 better / 4 worse, no page-count change)** but does NOT move business-plans/13, whose paragraph inherits 8pt spacing-after and so needs the `IsAnchorOnlyMark` branch widened too — and that branch's probe (agendas-minutes/11) covered an *explicit* spacing-after, a distinction `props.SpacingAfterPoints` cannot currently make.
