...
- **#25 Word's row-split trigger — LANDED 2026-08-07 (third attempt); measured residues below.** The full bundle shipped together: table-level fit routing (a table that does not fit the space left flows row by row, mirroring the whole-table move's slack exactly), the split-acceptance test (a first fragment offered only a region remainder must genuinely split — something continues AND the placed content fits — else the row moves whole), the vMerge tie rule (a continuation row never breaks from its predecessor; it stacks, overflowing and clipping, pinned by `CanonicalFragmenterTests.A_merge_continuation_row_stacks_rather_than_breaking`), the atLeast-floor fit exemption (the floor is drawn but does not participate in break decisions, pinned by `A_floored_row_whose_content_fits_stays_and_overflows`), and style-inherited `w:pageBreakBefore` with the inline `w:val="0"` override (the old parser comment claiming Word only honours it inline was refuted by probe). Probe evidence, mechanics and the two failed attempts' history: `docs/word-features.md` (Table row height / Page break before) and git history. Corpus: page counts 325/325/325, aggregate mean |Word−render| 7.799 → 7.631 over 41 changed pages — `business-plans/10` −0.6..−2.6 on both changed pages (its p5 break is the Heading1 style's pageBreakBefore, not table logic), `resumes/13` p3-p5 band structure now matches Word where the old render had wrong content, `business-plans/15` p8/p9 a wash inside its deep pre-existing divergence. Baselines regenerated for the four scenarios. Still open, each measured:
  ...
  - `business-plans/13`'s split-table tails sit ~8-11pt above Word's (p13/p14/p15/p20 drifted +0.1..0.38 with the routing; the repeated `w:tblHeader` block also renders 8-11pt shorter than Word's). Strongest candidates from the LibreOffice scan, both always-on Word-compat behaviours: `ADD_PARA_SPACING_TO_TABLE_CELLS` (a cell grows by its last paragraph's space-after — one missing space-after is exactly this size) and `MIN_ROW_HEIGHT_INCL_BORDER` (an atLeast `w:trHeight` floor includes the top border line + top padding + bottom padding; `exact` adds only bottom padding — tabfrm.cxx:5058-5096). Probe before landing.
