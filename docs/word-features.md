...
- **Test**: `brochures/01`, `menus/07`, `menus/09`, `newsletters/03`, `letters/05`, `resumes/03`, `resumes/18`, `inline_shape_arrows`
...
> **AI**: Other gaps: `a:srcRect` crops on group pictures are ignored, and a group rotation is not applied to its pictures. Markdown renders a group's pictures at the browser's default box because Markdown carries no size syntax — the same limitation the plain inline-image path has for any `width`/`height`-less SVG, and the reason the icon graphics look oversized in `md_result` snapshots.
