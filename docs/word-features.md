...
- **Test**: `brochures/01`, `menus/07`, `menus/09`, `newsletters/03`, `letters/05`, `resumes/03`, `resumes/18`, `inline_shape_arrows`, `inline_group_crop`
...
> **AI**: Other gaps: a group rotation is not applied to its pictures. Markdown emits the whole picture — it can express neither an `a:srcRect` crop nor a size, so a cropped group picture exports uncropped, and any `width`/`height`-less SVG renders at the browser's default box (why the icon graphics look oversized in `md_result` snapshots). Both limits are shared with the plain inline-image path.
