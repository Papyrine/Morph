...
>
> **A ~1pt residual remains and is NOT the stroke direction.** With the shift in place Morph's inner edge is invariant at 149 against Word's 147 — Word's border box sits about 1pt outside the nominal (text margin − `w:space`). Swept over `w:space` 0/2/4/8/12/20/28pt at 6pt width (`_probe_pbdrspace`), the implied constant measures 0.96 / 0.88 / 0.80 / 0.64 / 1.44 / 1.12 / 0.80pt: flat, no trend with space, mean 0.95pt, the scatter being pixel quantisation of a rule Word draws unantialiased. It moves the BOX rather than the stroke, so it is left as its own change.
