...
>
> **Cell scope keeps its own model** (`BorderStroke.CellBands`): the symmetric families repeat the declared width per line — `_probe_celldouble` / `_probe_celltriple` at four magnitudes — the thin/thick family divides it, and a too-small width floors to 0.75pt per unit (Word floors to its pixel, but Morph antialiases where Word does not, and at 0.5pt the gap closes by pixel phase). Bringing cell borders onto the grid is a separate adjudication. `BorderStroke.Extent` reports the drawn thickness in either scope so `Fragmenter.EdgeReserve` charges the flow what the border actually occupies.
>
