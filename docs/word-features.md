...
- **Fill resolution**: a shape may name no fill in `spPr` at all and take it from the style's `wps:style/a:fillRef` instead — Word writes exactly that for a template's accent rules, e.g. `resumes/12`'s coral bar under "Manager" is a `rect` with `<a:ln><a:noFill/>` whose colour lives only in `<a:fillRef><a:schemeClr val="accent1">`. `ParseInlineSingleShapeRun` falls back to that reference, and the standalone-inline gate admits a shape on the strength of it, mirroring what the stroke path has always done with `lnRef`. Without both, such a shape resolved no fill and was dropped before it could be drawn.
