...
- **Model**: `Footnote` record (id, flat text); `ParsedDocument.Footnotes`; the reference run carries the citation-order number as superscripted `Text` plus `FootnoteReferenceId`; `ParsedDocument.FootnoteTextSizePoints` / `EndnoteTextSizePoints` carry the resolved `FootnoteText` / `EndnoteText` style sizes, which the appendix sets the note bodies in (Word's built-in 10pt when the styles part defines neither)
