...
> **Consumers**: Word's built-in default for a document with no docDefaults is Calibri 12pt (Word-probed; see `DocumentParser.builtInDefaultFontFamily`); a document that declares docDefaults but omits `w:sz` gets the spec default 10pt. Half-point values from OOXML are automatically converted. Word lays text out on per-glyph pixel advances at the em rounded onto its 120-dpi grid, memoized per half-point size in the `src/Fonts/*.wordadvances` sidecars (`FontMetrics.WordAdvances`) — live since 2026-08-30 for the five Calibri faces and since 2026-09-06 for Aptos (four faces), Arial (four), Avenir Next LT Pro, Century Gothic, Segoe UI (two each) and Franklin Gothic Book, twenty faces in all. Those are the GDI-compatible widths of compatibility mode 14 and older (162 of the 334 corpus packages, every settings-less one among them — the settings part has to be found through its relationship, since resumes/16 keeps its mode 15 declaration in `settings2.xml`). A mode 15 document lays text out on DirectWrite's widths instead: a hinted integer where the font grid-fits at that pixel size (Segoe UI 10pt 's' is 7px on both tracks) and fractional where it does not (Segoe UI 12pt 's' is 8.263px against GDI's 9, with the XPS showing pixel-snapped origins and the residue landing on the spaces), the inter-word space fractional too — memoized in `*.wordadvances15` sidecars (`FontMetrics.WordAdvancesMode15`; `WordAdvancesFor` picks by the document's mode, and `CanonicalTextMeasurer.WordPixels` rounds the space only on the GDI track). `scripts/generate-word-advances.py` regenerates one face per key (`<key>@15` for the mode-15 table, which needs `w14:ligatures none` in the probe or a run of f's ligates) and refuses a probe Word rendered in a substitute face (it reads the family names out of the XPS's embedded fonts); a face with no sidecar measures on the linear fallback, and the space-compression wedge (Kerning, below) fires only on sidecar-backed faces, on either track. The mode-15 table ships for the Calibri five (business/04 and letters/04 went band-for-band Word); the tables generated for the other fifteen faces are held back, because two mode-15 rules are still unmodelled — Word floors a kerned pair's first glyph to a whole pixel even for a −0.08px kern (`_probe_otf`, eight of eight pairs on cover-letters/03's first line; Morph's `KernPairDelta` rounds) and Aptos with kerning off ceils the same glyphs it floors with kerning on — so Aptos measured one word wider than Word on every kerned line of cover-letters/03 and Century Gothic put two lines on newsletters/11's page 2 that Word does not (`src/todo.md` #43). The four embedded Aptos faces carry their sidecars as assembly resources (`EmbeddedFonts.WordAdvances`): the resolver's bundled seed answers before the font directory, and until 2026-09-06 it loaded from bytes with no sidecar at all, so every Aptos document measured on the linear fallback whatever `src/Fonts` held.
...
Border drawn around an individual run (per-run rectangle, not paragraph-level). It reserves space in the LINE: the declared stack plus `w:space` above and below the font's line box, so a bordered line is taller and its baseline lower than a plain one.
...
>
> **The leading-space rule (2026-09-05, `_probe_runbdr4`).** `_probe_runbdr3` put the vertical reserve on a whole-line run, a whole-line run plus its mark, two bordered runs filling the line, a bordered first run and a bordered last run alike (a 10pt line grows to 15.6 with a 2.25pt/1pt single), while `_probe_runbdr2`'s mid-line boxes grew nothing and drew their rules INSIDE the font's line box. The sweep that separated them varied one thing at a time — size 0.75 to 4.5pt, space 0 to 8pt, first / mid / last — and the only variable that mattered was the run's first character: `" boxed "` draws inside (top band on the line top, bottom band ending on the line bottom, the 10pt line staying 12.6) at every size and position, `"boxed"` reserves (16.2 at 2.25pt/1pt, 18.0 at 4.5pt/1pt, 22.8 at 2.25pt/8pt). `BorderStroke.RunBorderReserves` keys on that leading space: the measurer charges no pad for such a run and the painters stroke its box inward from the line box (`RunBorderBox` with `reserves: false`); the glyph inset applies either way. `border_style_variants` section 3 is all leading-space runs.
...
> **Contributors**: Auto mode multiplies the font's full hhea line box (ascent + descent + line gap) with no extra floor or leading correction, verified against Word XPS baselines. Document grid line pitch enforced when >= 20 page break markers detected. An empty paragraph's line takes the paragraph mark's style-resolved formatting (`ParagraphProperties.ParagraphMarkRunProperties`, from `w:pPr/w:rPr`), matching Word. Inside a table cell, header or footer the resolved mark is omitted (its line-height rules are entangled with exact-row clipping and the end-of-cell mark collapse), and the mark's size and face come from `ParagraphMarkFontSizePoints`/`ParagraphMarkFontFamily` instead — resolved through the style chain even for a bare `<w:p/>` with no pPr at all (2026-09-06, `BareParagraphMarkTests`): letters/13 separates its cell-held letter paragraphs with such spacers under a Posterama 11 docDefaults, Word's XPS steps 29.42pt across each (two 14.7pt lines), and measuring them at the record's default face lost 1.2pt per spacer until the body sat a line up the page by its signature. See `TextRenderer` line spacing logic.
>
> **The line pitch itself is NOT grid-quantised; each baseline snaps to the 120-dpi grid as the fractional pitch accumulates.** Word-measured 2026-08-30 (`_probe_pitch2`, 45-line Calibri blocks across twelve sizes and three multipliers, baselines read from XPS `Glyphs` origins): every block's 44-gap mean pitch equals the NOMINAL fractional pitch (12pt at line 278: mean 16.976pt against nominal 16.968; 13pt single: 26.47px mean, refuting the rounded-em model's 26.85), while consecutive deltas alternate between the two adjacent whole-pixel values (16.8pt/17.4pt at 12pt-278) — the signature of accumulate-then-round-nearest, with a small ~0.025pt/line creep in the XPS numbers. Morph's unsnapped fractional advance therefore matches Word's cumulative position to under half a grid pixel at every line, and no per-line pitch rounding should ever be introduced — the modal delta (16.8pt) is an artifact of the alternation, not the pitch.
>
...
>
> The height is the natural pitch times the multiple throughout (h/natural 0.598 … 1.504). The ascent is NOT: **expanding leaves it at the natural value and puts every extra point below the baseline**, while **compressing scales it with the box** (a/natural 0.595 / 0.689 / 0.811 / 0.906 against the multiple). Keeping the natural ascent in both directions — which is what this did until 2026-08-12 — leaves compressed text `ascent × (1 − multiple)` too low inside its own box: 9.2pt on `business-plans/13`'s 0.8× cover title, whose ink measured exactly 19px low before and lands within 1px of Word after (Skia 1251–1328 and PDF 1252–1328 against Word's 1252–1329). Worth **Skia −0.195 / PDF −0.192 over 38 scenarios**, no page-count change; ImageSharp read +0.023 at the time for a painter reason, not a layout one — its painter top-anchored text and subtracted SixLabors' own ascender, which is not where SixLabors puts the baseline either; since 2026-09-04 it draws with `TextBaseline.Alphabetic` at the engine's baseline like the Skia and PDF painters.
>
> One residue the same probe measured but did not fix: **Word's first line on a page is 0.6pt shorter than the rest**, which the probe sees as a one-off at the top of the page. (Its other residue — a natural ascent ~0.08 em large — is the baseline rule below; the 44.53 in the ascent column is that probe's reference-line subtraction, and reads 45.0 against the page margin.)
>
> **Where the baseline sits inside the box: the line box minus the descent, with the line gap ABOVE the text** (`FontMetrics.BaselineAscentUnits`, pinned by `BaselineAscentContractTests`). Word-measured 2026-09-04 with `_probe_baseline` / `_probe_baseline2`: 23 faces at 24, 48 and 96pt, one face and size per page with every spacing zeroed, the first baseline read from the XPS `Glyphs` origin against the top margin. The descent is the same metric family the box is built from — `usWinDescent`, or `−sTypoDescender` when USE_TYPO_METRICS is set — so the baseline honours the flag exactly as the pitch does. Before this the baseline was `usWinAscent` for every font, a rule that had coincided with Word only on faces with no leading. Pixels on Word's 120-dpi grid, first baseline below the margin, Word against the two candidate models:
>
> | face | flag | 24pt Word / box−desc / usWinAscent | 48pt | 96pt |
> | --- | --- | --- | --- | --- |
> | Aptos | typo | 38 / 38 / 40 | 75 / 75 / 81 | 150 / 150 / 162 |
> | Calibri | win | 38 / 38 / 38 | 77 / 77 / 76 | 152 / 152 / 152 |
> | Arial | win, 67-unit hhea gap | 38 / 38 / 36 | 75 / 75 / 72 | 150 / 150 / 145 |
> | Times New Roman | win, 87-unit gap | 37 / 37 / 36 | 75 / 75 / 71 | 149 / 149 / 143 |
> | Bahnschrift | typo, 410-unit typo gap | 40 / 40 / 40 | 80 / 80 / 80 | 159 / 159 / 159 |
> | Gabriola | typo, 0.7 em typo gap | 55 / 55 / 48 | 111 / 111 / 95 | 221 / 221 / 191 |
> | Baskerville Old Face | win, no gap | 36 / 36 / 35 | 70 / 70 / 71 | 142 / 142 / 141 |
>
> Every one of the 68 face/size rows lands on `round(box px) − round(descent px)` except three that Word places one pixel higher (Verdana 96pt, Book Antiqua 48pt, Century Schoolbook 96pt — a .53–.59 descent fraction rounded down where Aptos's .54 rounds up; no single threshold on either term fits all 68, and VDMX is not it either: Trebuchet's VDMX yMax is 79 at 80 ppem where Word sits at 75). `round(ascent + gap)` misses ten rows, Calibri 48pt among them (77 against its 76.17). Bahnschrift is the trap that kept `usWinAscent` alive: its typo ascender plus typo gap equals its usWinAscent, so a flagged variable font agreed with the wrong rule. The engine quantises the descent to the grid and keeps the box fractional (`FontMetrics.BaselineAscentPoints`), since the box's snap is the per-line accumulation described above. Compressed multiples still scale this ascent: Word's 0.9× Aptos 48pt baseline is 67.2px against 75 × 0.9 = 67.5, not (0.9 × 97.66 → 88) − 23 = 65. Landing it, with the ImageSharp painter moved onto the engine's baseline in the same change, read **Skia −0.214 / PDF −0.209 / ImageSharp −1.710** summed over 661 pages (161 / 171 / 333 scenarios closer to Word against 146 / 149 / 59 further, the largest single regression +0.012), page counts unchanged on all three.
>
...
>
> **Stroke direction (2026-08-29).** Word strokes a paragraph border OUTWARD from the border box — the rule's inner face sits ON the box and the width grows away from the text — where the engine centred it. Probed at three widths in one render (`_probe_pbdr`: left-only borders at `w:sz` 24/48/96 = 3/6/12pt, `w:space=0`, 150 DPI), Word's inner edge is invariant at x=147 while its outer edge moves 141 → 135 → 123; a centred stack would have moved both, which is what makes this a MODEL result rather than a constant fitted to one width. Morph drew 147-152 / 144-155 / 138-161 against Word's 141-147 / 135-147 / 123-146, and now draws 144-149 / 137-149 / 125-149. `BorderStroke.OutwardShift` shifts the whole stack by half the INNERMOST band's thickness, which puts that face on the box without disturbing the gaps the multi-line families were measured with. Two scopes are exempt. A CELL edge has its own placement (`BorderStroke.CellEdgeLines`: the horizontals hang down from the grid line, the verticals are centred on it — see Cell Borders). A PAGE frame (`w:pgBorders`) arrives already centred — `PageBorders.EdgeRect` adds half the stroke on purpose, Word-measured on page_borders/01 — and grows INWARD from the paper edge, where "outward from the box" has no meaning; shifting it regressed that scenario by +0.0096 AE / -0.033 SSIM before `Scope.Page` was added, which is how the exemption was found. The fix also reconciles paint with reserve: `Fragmenter` already charges the full `BorderStroke.Extent` from the box, which a centred stack only half used.
>
> **The box overhangs the nominal on the sides, by a constant (2026-09-04).** With the shift in place Morph's inner edge had been invariant at 149px against Word's 147 — Word's border box sits outside the nominal (text extent − `w:space`), and a `w:space` sweep at 6pt width (`_probe_pbdrspace`, PNG-read) had put the left offset at a flat ~0.95pt. Two XPS-read probes settled it as a MODEL rather than a fitted number (`_probe_pbdrwidth`: 1/3/6/12pt widths at `w:space` 8; `_probe_pbdrside`: `w:space` 0/8/20, a 720tw left indent, a 720tw right indent, and a second section with 36pt/108pt margins), reading each rule's inner face straight from the border `Path` rectangles:
>
> | side | nominal − Word inner face | readings |
> | --- | --- | --- |
> | left | **1.0pt** outside | 0.95 / 0.95 / 0.95 / 0.90 / 0.95 / 0.98 (1.12 at `w:space` 0) |
> | right | **1.5pt** outside | 1.53 / 1.53 / 1.55 / 1.53 / 1.50 / 1.50 (1.73 at `w:space` 0) |
> | top | nominal | inner face 7.82pt above the line top at `w:space` 8, every width |
> | bottom | nominal | inner face 8.0pt below the line bottom, every width |
>
> Invariant across four stroke widths, three spaces, both indents and a different margin, so `Fragmenter.BorderBoxLeftOutset` / `BorderBoxRightOutset` widen the box by 1pt and 1.5pt on the paint side only — the vertical faces are nominal, so the flow reserves are untouched. The 1pt border in that probe is the one width Word does not draw at its declared size: 0.6pt (one 120-dpi pixel), where 3/6/12pt draw at 3.02/6.0/12.03. The HTML export's `padding` still carries the bare `w:space`. Landing it moved 20 pages per backend by a hair (Skia −0.0010 / PDF −0.0008 / ImageSharp −0.0022 summed AE; `paragraph_borders` and `html_css_margin_padding` closer on all three), and put `border_style_variants`' outer faces exactly on Word's (left 143px, right 1095–1098px at 150 DPI); the residual there is the double style's inter-band gap (Word 3px, Morph 1px), a multi-line-family question, not the box.
...
Marks a paragraph's indents as inside/outside for mirror printing. Word-measured 2026-08-30 (`_probe_mirror2`, hanging and firstLine swept on both page parities, right edges bracketed by short-word wraps; `complex_spacing`'s Combination 7 confirms every number on a real document): an EVEN page keeps the declared indents; an ODD page mirrors the box — with F = firstLine − hanging, every line's right inset becomes `left + F`, the continuation left becomes `right − min(F, 0)`, and the first-line delta F itself is unchanged (in field terms `left' = right + hanging`, `right' = left + firstLine − hanging`; F = 0 degenerates to a plain left/right swap). The 2026-08-19 probe had concluded the indents were dropped entirely — it read only line STARTS, every one of which the mirror transform also produces, and the right-edge measurements refute the drop. The `Fragmenter` applies the transform at placement (`MirrorForPage`), deciding parity greedily from the page the paragraph starts on; the text exporters reflow with the declared indents.
...
>
> **And how wide the box may be (2026-09-05, `_probe_pct12` / `_probe_pct15`: 100%, 50%, dxa and auto tables under 108-twip, 720-twip and per-cell 1in margins).** In modes 12 and 14 the column bounds the TEXT extent, so a `w:tblW` percentage is taken of the column plus the first cell's left margin and the last cell's right margin and the border box hangs outside the column by them: 100% reads 479.25 (468 + 5.4 + 5.4) under the default margins, 540.5 under 36pt ones, 545.4 with a 1in first-cell margin against the default last; 50% is half of that outdented box (270.3); an over-wide autofit table squeezes to the same box, while a grid that fits the column keeps it (`compatibility_mode_14`'s 539.5pt grid on a 540pt column); a dxa width is the border box on either mode. The left margin joins only when the border hangs left of the margin — an undeclared indent over the table's own `w:tblCellMar` puts the border ON the margin and the box runs the right margin past the column alone (`table_autofit_no_widths` reads 474 on 468 in Word). In mode 15 the column bounds the border box (468 at 100% whatever the margins). `TableProperties.WidthOverhangPoints` carries the margins for modes 12/14 and `TableLayout.CalculateColumnWidths` takes its percentage targets and squeeze limit from the outdented box, which is the 23px `table_grid_styling_padding` was short at its right edge (Word 1136, Morph 1113).
...
> - A CELL border follows the SAME rule: line = `w:sz`, gap = `w:sz`, centre-to-centre exactly `2 × w:sz`. `_probe_celldouble` measured it at four magnitudes (150 DPI: 13px lines with a 12px gap at `sz=48`, 6/6 at `sz=24`, 3/2 at `sz=12`, 1/2 at `sz=6`), and `_probe_celltriple` confirmed `triple` the same way at `sz=12/24/48` (three `sz`-wide lines with `sz`-wide gaps). An earlier reading off `table_default_style` at `sz=12` ("the declared width as a TOTAL") was settled at a size where the two hypotheses are 2px apart and antialiasing decides the answer — exactly the amplification failure mode this file warns about; at `sz=48` the readings are 12px apart and unambiguous. The scopes differ in PLACEMENT, below.
>
> **The PARAGRAPH families are settled on Word's own grid (2026-09-04, XPS-read).** `_probe_bands` declared fifteen styles as left-only borders at `w:sz` 12/24/48/96 and `_probe_bands2` added four-sided boxes and odd widths; the border rectangles in Word's XPS give every band's edges losslessly. Every band is a whole number of 120-dpi pixels (0.6pt), and the declared width is FLOORED to that grid with a one-pixel minimum — `sz=9` (1.875px) draws 1px, `sz=13` (2.71px) 2px, `sz=28` (5.83px) 5px, so a 1pt rule is 0.6pt and a 1.5pt rule 1.2pt. With W that floored width in pixels (`BorderStroke.ParagraphBands`, pinned by `BorderStrokeTests`):
>
> | family | bands, in page order (top→bottom or left→right) | measured at 6pt |
> | --- | --- | --- |
> | `double` / `triple` | W, gap W, W (…) | 6.0 / 6.0 / 6.0 |
> | `thinThickSmallGap` | thick W, gap 1px, thin 1px | 6.0 / 0.6 / 0.6 |
> | `thinThickMediumGap` | thick W, gap W/2, thin W/2 | 6.02 / 3.0 / 3.0 |
> | `thinThickLargeGap` | thick 2px, gap **W**, thin 1px | 1.2 / 6.0 / 0.6 |
> | `thickThin*` | the same, reversed | |
> | `thinThickThin*` | thin, gap, thick W, gap, thin — the same units | |
> | `threeDEngrave` | flank at 0.33, W at 0.41, flank at 1.0 of the colour, touching — a flank reserves 1.5pt and draws ⌊W/2⌋ pixels, one to two (0.6 / 0.6 / 0.6 at `border_style_variants`' 0.75pt) | 1.2 / 6.0 / 1.2 |
> | `threeDEmboss` | the same shades reversed | |
> | `inset` / `outset` | one band of W, unshaded | 6.0 |
>
> **The flow reserves the declared stack in points; only the paint floors.** `_probe_reserve` (a bordered paragraph between two plain ones, Calibri 12, per-edge reserve read from the XPS baselines) put every reserve at the unfloored point stack: a 1pt single reserves 0.97pt while drawing 0.6, a 3.5pt single 3.37 while drawing 3.0, `double` at 1.5pt 4.56 (three 1.5s) while drawing 3.6, `triple` at 1.5pt 7.56, `thinThickSmallGap` at 3pt 4.58 (3 + 0.75 + 0.75) while drawing 4.2, `thinThickLargeGap` at 3pt 5.17 (0.75 + 3 + 1.5) while drawing 4.8, and the 6pt groove 9.08 (1.5 + 6 + 1.5) while drawing 8.4. So the "one pixel" and "two pixel" units above are really 0.75pt and 1.5pt — `sz=6` and `sz=12` — that floor to one and two grid pixels when drawn. The first landing attempt reserved the drawn stack and drifted `html_css_borders`' thirteen boxes 9px up the page; `BorderStroke.ParagraphLayout` is the point stack, `Extent` sums it, and `ParagraphBands` floors it for the painters.
>
> Two things the earlier PNG-read model had wrong. The thin/thick family never "divided" the declared width: its thin line is a fixed 0.75pt, its large gap IS the declared width, and its medium units are half of it — which is why `thinThickLargeGap` at 3pt reserved ~5pt (0.75 + 3 + 1.5 = 5.25). And the asymmetric families are laid out in PAGE direction, not inside-out: a four-sided `thinThickSmallGap` box draws its thick line at the smaller coordinate on every edge — outer on the top and left, INNER on the right and bottom — and the bevels shade the same way (2A2A2A / 353535 / 7F7F7F from an 808080 declaration, top-to-bottom and left-to-right, for the groove; reversed for the ridge). The painters therefore pass which edge they are stroking, and the right and bottom edges take the mirrored stack. The bevel block is W plus two flanks — 8.4pt at 6pt, not the 1.5×W = 9pt the single-width PNG reading gave.
>
> **Landing note.** 18 pages per backend moved; every scenario but one is neutral or closer (`html_css_borders` −0.0044 AE / +0.017 SSIM on all three, its thirteen rule thicknesses now 1–4px against Word's 1–4 where the old model drew 2–5). `border_style_variants` reads +0.031 AE / −0.09 SSIM per backend and was vetted rather than believed: its band thickness patterns now match Word's row for row ((6,1) and (2,6) pairs where the old model drew (2,4)), and its rule positions against Word improved on pages 2 and 3 (mean displacement 31.8 → 24.3px and 19.1 → 15.7px) — but its whole page sits ~75px above Word for a reason that predates this change (its `todo.md` entry), so thicker correct ink at that displacement scores worse than thinner wrong ink. Skia +0.024 / PDF +0.027 / ImageSharp +0.026 summed, all of it that fixture.
>
> **Cell scope on the grid (2026-09-05, XPS-read).** `_probe_cellw` (a `single` at `w:sz` 6/8/9/12/13/24/28/48/96 on 2x2 tables, Calibri 12, zero cell margins), `_probe_cellfam` (the fifteen families at 3pt and 6pt) and `_probe_cellmix` (cell margins, one-sided and conflicting edges) put cell borders on the same 120-dpi grid as paragraph borders: `BorderStroke.CellStack` is `ParagraphLayout` floored, band for band — a 1pt rule draws 0.6, a 1.5pt 1.2, a 3pt `double` 3/3/3, a 3pt `thinThickSmallGap` 3/1px/1px, a 6pt groove 2px/6/2px shaded 2A/35/7F from 808080 — with placement rules of their own (`BorderStroke.CellEdgeLines`, pinned by `BorderStrokeTests` and `CanonicalFragmenterTests`):
>
> - **A horizontal edge hangs DOWN from its grid line, and the row reserves the whole declared stack under it.** The table's top rule starts on the line the previous paragraph ended on and the first row's text sits a full stack lower — 6pt under a 6pt rule, 9pt under a 3pt `double`, 4.5 under a 3pt `thinThickSmallGap`, the reserve unfloored (a 1pt rule reserves 1pt and draws 0.6, a 0.5pt one reserves 0.5 and draws 0.6). Each interior rule hangs into the row below, whose content starts under it; the last row's bottom rule starts a stack above the row's bottom face. A 2-row table is three stacks taller than its content. `TableHeightCalculator.HorizontalBorderWidth` charges the stacks (the charge was the bare `w:sz` before, 3pt for that 9pt `double`), `PlacedCell.BottomEdgeInset` tells the painter where the last row's stack starts, and `Fragmenter.BuildRow` spends the reserve ABOVE the content — before this landing the engine charged the row but left the space under the text, so every bordered row's text sat a rule too high and the bottom rule straddled the table's foot a half-width low.
> - **A shared line is charged whichever row declares it, at the wider.** A 6pt bottom on the first row alone, or a 6pt top on the second alone, both open 6pt between the rows; 3pt against 12pt opens 12 and draws 12. The old charge read only the lower row's resolved top, so a header row's per-cell bottom rule cost the row under it nothing.
> - **A vertical edge is centred on its grid line, and the content sits at max(margin, half the stack).** Word biases the drawn stack by up to half a pixel (an outer 5px rule is 3px outside and 2px inside; an interior one 2/3), below what 150 DPI resolves, so the painters centre it. Against Word's 5.4pt default margin a 6pt rule (half 3) leaves the text at 5.4, a 12pt at 6 and a 24pt at 12 — the max, never the sum (`TableLayout.HalfBorder`, now half the declared stack: 4.8 for a 3pt `double`). Vertically it IS the sum: a 6pt top margin under a 6pt rule puts the text 12pt down.
> - **The asymmetric families keep page order on every edge** — the thick line at the smaller coordinate on all four sides, the bevels shading top-to-bottom and left-to-right — so unlike the paragraph painters nothing is mirrored for the right and bottom.
>
> The page scope (`BorderStroke.PageBands`) keeps the earlier PNG-probed model; bringing page frames onto the grid is a separate adjudication.
>
> **Landing note (2026-09-05, with the vertical-merge rule above).** Skia −0.547 / PDF −0.505 / ImageSharp −0.503 summed AE over 662 pages (≈147 pages closer, ≈62 further, per backend), +1.95 to +1.96 summed SSIM, no page-count changes. The largest movers are the bordered-table fixtures and templates: newsletters/09 −0.07 (the merge rule), border_style_variants −0.036, table_indent −0.032, table_multipage −0.027, labels/08 −0.028 with its SSIM 0.71 → 0.91 (the 3pt `double` grid now reserves its 9pt stacks and hangs each rule under its line), business-plans/12 −0.022. The pages that read worse are thin-rule tables where the grid-floored 0.6pt rule is heavier ink than the 0.5pt it replaces (business-plans/13 and /15 at +0.01 to +0.026 on alternating pages that move a few pixels, newsletters/01 and /04 at +0.01) — the new-ink penalty, vetted on crops, the rows themselves within a pixel or two of where they were. table_default_style_outer_borders' bottom rule, 8px high before, now sits on Word's.
>
> **Bevels**: `threeDEngrave`/`threeDEmboss` are not a line/gap layout at all — Word draws ONE contiguous block in three shades (the table above), in opposite order for the two. `inset`/`outset` carry no shading whatever: at `sz=48` both are solid at the declared grey. First measured with `_probe_bevel` at 6pt, and the size mattered — at the 0.75pt the fixture originally used, every one of these collapses to 1-2px where antialiasing is indistinguishable from a light line, and reading `outset` at that size suggested a highlight that does not exist.
>
...
> **Contributors — a DASH is a break opportunity (2026-09-04)**: the wrap broke only at U+0020 and `\n`, so any hyphenated token was unbreakable. Word breaks after a dash, and the break falls AFTER it — the dash stays on the upper line. Probed by squeezing a token in an autofit table's first column against a second column long enough to take every spare point, then comparing against the same token's natural single-line width (`_probe_hyphen`, natural → squeezed in points): `well-known-example-string` 128.05 → 78.95, `2024-2025-2026-2027` 104.55 → 61.45, en dash U+2013 119.45 → 83.30, em dash U+2014 132.95 → 88.00, against a solid 29-character word (128.10, unmoved), a slash-separated one (143.35, unmoved) and — the control that proves the effect is the character rather than the length — the NON-BREAKING hyphen U+2011 (116.85, unmoved). A LEADING dash breaks: `-Supercalifragilisticexpial` went 118.10 → 115.70, and that 2.40pt is exactly the hyphen's own advance, so Word put the lone dash on the upper line. A trailing dash needs no exemption, measuring as unbreakable on its own. `CanonicalTextMeasurer.SplitAfterDashes` implements it for both wrap paths (`TokenizeText` marks the pieces `BreaksBefore` so the word-gathering loop stops there; `WrapLines` mirrors it, which `CanonicalWrapAgreementTests` holds to). This is distinct from AUTOMATIC hyphenation, which inserts dashes at syllable points and is still unimplemented. The autofit minimum is what makes it load-bearing: a column's minimum is its longest unbreakable token, so seeded identifiers like `E2E-FinalisedActions-44b577b2` inflated one 700pt-wide table's Name column from a 81.0pt minimum to 132.0pt, which raised the min/preferred interpolation ratio and handed that column ~12pt taken from the other seven — re-wrapping 1,267 rows and losing 2 pages against Word across a 137-page document. Corpus effect: 17 scenarios moved, 46 page/backend pairs better against Word's references and 10 worse (worst +0.00026, noise), mean AE 0.04602 → 0.04507, with the wins concentrated in `table_autofit_no_widths` (−0.0147 on all three backends) and `table_grid_styling_padding` (−0.004).
...
> **Contributors — a band float is admitted whatever its `behindDoc`.** `Fragmenter.ResolveBandImages` used to match `FloatingImageElement {BehindText: true}`, which silently dropped every front-of-text picture anchored in a header or footer — including the commonest case there is, a right-aligned logo with `wrapNone`, for which `behindDoc="0"` is what Word writes. The band SHAPE branch beside it had already been widened for the same reason (cards/05's fold-guide rules); the image branch had not, and no corpus scenario carried a front-of-text band PICTURE, so nothing caught it. The gate was never a Word rule to begin with: `behindDoc` orders a float against the body TEXT and says nothing about the bands, which sit under the body either way — Word-probed in `docs/floating-art-pipeline.md` (`_probe_footerz`). Covered by `header_float_image/`, which pairs a `behindDoc="0"` and a `behindDoc="1"` picture in one header so the behind-only model cannot pass it.
...
- **Test**: `page_borders/`, spec test `PageBordersTests`, `BorderStrokeTests.Page_edges_are_paragraph_edges_on_the_grid_from_their_inner_face`

> **On the grid (2026-09-05, XPS-read on `_probe_pgbdr`: `single` at 0.75/3/6pt, `double` 1.5pt, `thinThickSmallGap` 3pt and `threeDEmboss` 3pt, at text and page offsets).** A page frame is the paragraph stack: the declared width floored to the 120-dpi grid (0.75pt draws 0.6, a 1.5pt `double` 1.2/1.2/1.2), the asymmetric families in page order on every edge (the thick line outermost on the top and left, innermost on the bottom and right), stacked outward from the inner face. In text mode that face sits exactly the declared space outside the text boundary (48.0 for space 24 on a 72pt margin, whatever the width); in page mode the OUTER face sits the space from the page edge (24.0), so `PageBorders.EdgeRect` returns the inner face one drawn stack in. The PNG-probed page model it replaces drew the declared width unfloored and the thin/thick family the other way round; the corpus carries only single page frames, so no baseline moved. The emboss shades Word draws for a black declaration (C0C0C0 / 606060 / 000000) are not the multiplicative 0.33 / 0.41 of the 808080 paragraph probe and stay unmodelled.
...
- **Layout**: the image's bottom sits on the baseline; `CanonicalParagraphMeasurer.ImageLineHeight` makes the line the image's height PLUS the text descent when the image is the taller. The line reserves the picture's layout box — the extent grown by `wp:effectExtent` on every side (`ImageEffectExtent`, `LaidOutImage.BoxWidth`/`BoxHeight`) — and `Fragmenter.MapImages` draws the picture inside it, offset by the left and top edges (`InlinePictureEffectExtentTests`)
- **Frame**: a picture's own `pic:spPr/a:ln` (`ImageOutline`, parsed by `DocumentParser.ReadPictureOutline` for inline and anchored pictures alike) is stroked by all three painters as a band lying entirely outside the picture's box
- **Test**: `inline_image/`, `multiple_images/`, `icon_with_text/`

> **Line height with an inline picture or shape (2026-09-05, XPS-read on `_probe_inline`: 6/24/48pt pictures and 4/12/30pt inline rectangles in a 10pt Calibri line).** The bottom of both sits on the baseline (a 24pt picture spans 125.7–149.7 against a 150.1 baseline, a 4pt rectangle 262.3–266.3 against 266.6), the line above sits the image height plus its own descent over the baseline (3.2 + 24 = 27.2, read 27.6), and the line below keeps the plain pitch (2.7 descent + 9.5 ascent = 12.2, read 12.0) — so a text line holding a picture is `image + descent`, not `max(pitch, image)`. Taking the max dropped the descent on every such line, which is why icon_with_text's paragraph after its inline star sat 10-13px high. The descent is the text runs' own: a paragraph holding ONLY the image keeps none under it — the first regeneration charged one to every image line and drifted the image-only stacks of cards/16, postcards/02, labels/13 and resumes/11 (card art, label icons, short inline rules) further from Word than the plain image height, so the rule is scoped to text-bearing lines. `_probe_inline2` then read the image-only case directly: a picture or inline rectangle alone in its paragraph makes a line of max(mark pitch, image height) — a 6pt picture or 4pt rectangle under a 12pt mark leaves the 14.65 mark line, a 24pt picture makes a 24.5 line, a 48pt one 47.9, a 30pt rectangle 29.9, a 24pt picture under a 24pt mark the mark's 29.3 — with no descent under it. Where the image sits inside that line was the last piece (`_probe_r12`, a mimic of resumes/12's cell, XPS-read the same day): the paragraph mark's space glyph — Word's baseline for the line — sits 14.8pt under the line top for a 3.6pt rectangle alone in a 12pt Calibri Light paragraph, the full 14.65 pitch, where a text line's baseline sits 11.4. So an image-only line puts its baseline, and the image's bottom, on the line BOTTOM; the engine had it on the mark font's ascent, 3.2pt high, which was resumes/12's coral rule (9px) and the same offset on every small image alone in its paragraph.

> **Frame and effect extent (2026-09-06, XPS-read on `_probe_picln2`: a 150 × 112.5pt picture at a 72pt margin under 12pt and 24pt `a:ln` lines, `algn` ctr and in, `wp:effectExtent` of 0, 12, 24 and 30pt, with and without the line, with and without `a:sp3d`).** The picture draws at the paragraph origin plus the effect extent's left and top edges (84pt for 12pt, 96 for 24, 102 for 30), the line reserves the extent plus all four edges — the caption's baseline under it moves down by exactly the bottom edge, 12, 24 and 30pt, whether or not a line is present — and the outline is stroked as a band lying entirely OUTSIDE the extent, from the picture's edge to the line width beyond it, for `ctr` and `in` alike (the 12pt line's path runs 66..228 around a 72..222 picture with its 12pt stroke covering 60..72; the 24pt line's 84..258 around 96..246). `sp3d` changes nothing geometric. newsletters/01's photos (7pt white line, 10.5pt effect extent each side) match: Word's frame box measures 273 × 167pt around a 261 × 154pt photo whose top sits 10pt below the cell content top, the engine's now within 3-4pt. That residue is the image-only line's height: the probe's picture lines measure 2.4-3.5pt taller in Word than the extent, the descent the corpus adjudication above declined to charge (cards/16, postcards/02, labels/13, resumes/11), so it stays recorded in `src/todo.md` under newsletters/01. Reserved effect extents were already honoured for inline shape groups (`ParseInlineShapeGroupRun` folds them into the box); anchored pictures keep their extent as the drawn box and the outline outside it, with no effect-extent reservation modelled.
...
- **Model**: `FloatingImageElement` with anchor enums, wrap type, position offsets, and the picture's `a:ln` frame (`Outline`, stroked outside the box exactly as an inline picture's — see Inline Images). The picture's `pic:spPr` geometry crop rides along as `ClipToEllipse`/`ClipSubpaths` — Word's circular picture style (brochures/03) clips the draw in Skia (`ClipPath`), PDF (`IntersectClip`) and ImageSharp (pre-clipped ellipse composite; custGeom unclipped there); rotated pictures exempt.
...
- **Rotation**: a text box with an `a:xfrm` rotation lays its content out in its own box and the painters draw it rotated about the box centre with the chrome (`PlacedRotatedGroup`, 2026-09-05) — before that only the chrome turned, and labels/06's "ADMIT ONE" stubs lay flat across their ticket seams
...
- **OOXML**: `footnotes.xml` part, `w:footnoteReference` in document, `w:footnoteRef` in the note body, the `separator` / `continuationSeparator` entries
- **Model**: `Footnote` record (id, flat text, and the body as parsed `Elements`); `ParsedDocument.Footnotes`; the reference run carries the citation-order number as superscripted `Text` plus `FootnoteReferenceId`; `ParsedDocument.FootnoteSeparator` / `FootnoteContinuationSeparator` hold the separator paragraphs (runs stripped); `DocumentNotes` bundles all of it for the layout engine
- **Parse**: `DocumentParser.ExtractFootnotes()` reads `FootnotesPart` after the body, so the citation numbers are known: each user note's body parses as ordinary paragraphs (`ParseNoteBody`, the header path) with its `w:footnoteRef` run drawing the note's number in the run's own formatting (Word's `FootnoteReference` style supplies the superscript; a note authored without the run shows no number, as document_capture/01's reference does). Reference marks number by citation order (`footnoteCitationNumbers` — footnotes.xml ids start at 2, so ids are never the display number; repeat citations reuse their number), format via the document's `w:footnotePr`/`w:endnotePr` `w:numFmt` (Word defaults: decimal footnotes, LOWERCASE-ROMAN endnotes — an undeclared endnote mark reads "i", not "1"; document_capture/01, 2026-08-20), and render through the ordinary superscript path in every backend. The separator entries parse to paragraphs with their runs stripped (`ParseSeparatorParagraph`): Word sizes a separator by the paragraph's STYLE and ignores every run size the entry declares, on the run or on the mark — `_probe_fn_i`/`_j` (30pt and 10pt marks) rendered identically to the unmarked `_probe_fn_a`, while `_probe_fn_l`/`_m`/`_n` moved exactly with docDefaults (20pt, 40pt) and a 40pt Normal. An empty entry, or none, gets Word's own: an empty single-spaced paragraph with nothing after — document_capture/01 declares empty entries and Word draws the rule anyway. The exporters branch on `FootnoteReferenceId` before `Text` and keep their own marker emission.
- **Layout** (`Fragmenter`, 2026-09-05, XPS-read on fourteen `_probe_fn_*` fixtures): a cited note's rows — each paragraph's spacing-before, its lines wrapped to the column, its spacing-after — stack in the column's page-bottom **area** (`NoteArea`, one per column), whose bottom is the content bottom and whose top is the effective bottom the body breaks against (`contentBottom` rises by the area, `pageContentBottom` stays the margin). The area opens when the citing line lands (`CommitLineNotes` → `CommitFootnotes`), or when a table row carrying a cell reference is placed (`CommitRowNotes`, `_probe_fn_k`: two cell references and one after the table stack 1, 2, 3). Measured laws:
  - **The body stops where the area begins.** `_probe_fn_d`: 44 twelve-point lines fit the page alone; with a two-paragraph note on line 5 and another on line 30, page 1 takes 40, the area 58.6pt (separator 14.65 + three 14.65 lines), the rule at 668.9pt and the last note baseline at 717.55 with its descent on the 720pt margin.
  - **A citing line keeps its note's first line.** `_probe_fn_e`: a six-paragraph note cited on line 42 of 44 — page 1 keeps line 42, the separator and note paragraph 1 (32.7pt left, 29.3 needed); paragraphs 2-6 continue on page 2's bottom under a full-width continuation rule. The fit loop charges a citing candidate its whole box plus `NoteMinimum` (the separator when the area is empty, then through the first note line), and the candidates after it see the bottom risen by the note.
  - **A long note displaces the body after its reference.** `_probe_fn_f` (70-paragraph note cited on line 3 of 10): page 1 holds lines 1-3, the separator and note paragraphs 1-40 down to the margin; page 2 opens with lines 4-10 at the top and paragraphs 41-70 at the bottom under the continuation rule. `_probe_fn_g` (40 paragraphs, 60-line body): the whole note fits page 1 with lines 1-3, and lines 4-47 open page 2. So the note takes what it needs from the citing line down, split at a line boundary (`AcceptNoteRows`), the remainder pulled into the next page's first-column area before that page's body flows (`PullPendingNotes`); a note still spilling past the last body page continues onto pages of its own.
  - **The separator rule is the mark font's strikethrough stroke, two inches long.** Every fixture draws it 72.075 → 216.2pt (144.125) from the column left on a 468pt column; the continuation rule spans the column (468.45). Its top sits `yStrikeoutPosition` above the separator paragraph's mark baseline and it is `yStrikeoutSize` thick, on the 120-dpi grid: Aptos 12pt 4.2 / 0.6pt measured against 4.28 / 0.6 predicted, Calibri 20pt 4.8 / 1.2 against 5.0 / 1.2, Calibri 40pt 10.2 / 2.4 against 10.0 / 2.4, Times New Roman 40pt 10.2 / 1.8 against 10.35 / 1.8 (`_probe_fn_a`, `_l`, `_m`, `_o`; `CanonicalParagraphMeasurer.StrikeoutBand`, `FontMetrics.StrikeoutPosition/Size`). The separator paragraph's line box is its style's mark line — 14.65pt for the no-styles Aptos 12 default — and the rule is emitted as a black `PlacedShading` band (`SeparatorRule`), which every painter already fills.
  - document_capture/01 renders band-for-band Word: the endnote rule at 320px against 320, the footnote rule at 1434 against 1435, both note lines within 3px.
- **Test**: `document_capture/01`; `FootnoteLayoutTests` pins the pinning, the strikethrough rule, the body stopping at the area, the split with the continuation rule, the citing line moving with its note's first line, repeat and cell citations, and the endnote block.

> **AI**: Not modelled: `w:footnotePr/w:pos="beneathText"` (the corpus is all `pageBottom`), a table inside a note body, widow/orphan pairing inside a split note, and the ordering when a second reference lands on a page whose earlier note is already spilling (its note queues behind the spill). Footnotes in a multi-column section stack under their own column, and a spill continues in the next page's first column.
...
- **OOXML**: `endnotes.xml` part, `w:endnoteReference` in document, `w:endnoteRef` in the note body
- **Model**: `Endnote` record (id, flat text, the body as parsed `Elements`); `ParsedDocument.Endnotes`, `EndnoteSeparator` / `EndnoteContinuationSeparator`
- **Parse**: `DocumentParser.ExtractEndnotes()` reads `EndnotesPart` exactly as the footnotes part is read — note bodies as paragraphs with the `w:endnoteRef` run drawing the citation number in the endnote format, separators as run-stripped paragraphs
- **Layout** (`Fragmenter.PlaceEndnotes`, 2026-09-05): the cited endnotes flow after the last body element, in citation order, as ordinary paragraphs under the endnote separator — Word ends the document with them rather than pinning them (document_capture/01 and `_probe_fn_a`: the rule follows the last body line's box and the note follows the rule's mark line). The separator's spacing-before collapses with the body's last after-spacing like any paragraph's; the separator and the first note line stay together, and when they miss the space left — strictly, not by the flow's 2% slack — the block opens the next page at the margin top (`_probe_fn_h`: 43 lines leave 18pt, the block needs 29.3, Word's page 2 opens with the rule at 79.5pt). The rule is the same strikethrough stroke, two inches long.
- **Test**: `document_capture/01`, `FootnoteLayoutTests`

> **AI**: Section-end placement (`w:endnotePr/w:pos="sectEnd"`, one endnote group per section) and per-section restarts are not modelled; everything flows at document end.
