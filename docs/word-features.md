...
> | 48 | +27.9% | +48.7% |
>
> Word's bold adds **~26%** ink on average; Skia's synthesis adds **~46%** — roughly 1.8× heavy from 16pt up, agreeing only at 8–12pt. Every ImageSharp attempt was calibrated against *Skia's* ratio, so all of them inherited that error. A fifth attempt should target Word's ~26%. This cannot be tuned on the Skia side — `SKFont.Embolden` takes no width, so changing it means hand-stroking glyphs there too. And before fitting a curve, gather more data: those per-size implied fractions are non-monotonic (0.046, 0.035, 0.018, 0.014, 0.025, 0.019), one font, ink thresholded at a fixed cutoff that behaves differently across sizes.
...
> **AI**: Skia and ImageSharp map luminance onto the two-colour ramp (`out_c = dark_c + L·(light_c − dark_c)` — a colour matrix in both; ImageSharp applies `Grayscale()` then `Filter`; an unresolved dark end defaults to black and light end to white, so the Recolor-gallery `(darkColor, white)` form is the historical single-colour behaviour). letters/02's frame pairs `prstClr black` with a tinted accent — treating the first RESOLVABLE colour as the dark end painted that frame with its highlight colour. Greyscale remains the fallback when neither colour resolves. The PDF backend applies no picture effects at all (Morph.Pdf has no pixel pipeline — PdfSharp only), the HTML export ships original bytes, and group-shape pictures don't carry effects.
