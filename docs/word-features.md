...
> **Consumers**: If a document uses a font not installed on the system, Morph falls back through a chain of alternatives. Set `ConversionOptions.FontFallback` to provide custom mappings. Default font is Georgia 11pt — Georgia ships on Windows, macOS, and most Linux distributions out of the box, unlike Word's newer default (Aptos, Microsoft 365 only). Override globally via `DefaultFontSettings.DefaultFont` (must be set before the first render — throws afterwards), or per conversion via `WordRender.ConversionOptions.DefaultFont`.
